...
def extract_object_docstring(dotted_path: str) -> str:
    ...
    try:
        ...
        # pandas_ta aliases a module with a function
        # 'pandas_ta.volatility.aberration'
        # AttributeError("'function' object has no attribute 'aberration'")
        return ""
    return get_first_line(getattr(obj, "__doc__", ""))


def extract_mod_path(dotted_path: str) -> str:
...
def fixed_init(self, app):
    ...
    self.env.filters["extract_mod_path"] = extract_mod_path
...
def write_doc(self, docname: str, doctree: nodes.document) -> None:
    ...
    class ExtractMeta(nodes.GenericNodeVisitor):
        ...
        def default_visit(self, node):
            ...
            if isinstance(node, docutils.nodes.meta):
                self.metas[node.attributes.get("name")] = node.attributes.get("content")
