...
.. _order-flow-transaction-clock-normality:

Order Flow, Transaction Clock, and Normality of Asset Returns
------------------------------------------------------------

Thierry Ané and Hélyette Geman (Journal of Finance, 2000) provide the empirical and theoretical case for *why* sampling by trade count — the tick clock — produces better-behaved returns, which is the statistical justification underneath tick and information-driven bars. The goal is to recover normality of asset returns through a stochastic time change. Clark (1973) had addressed this by representing the price as a subordinated process with volume as a lognormal subordinator; Ané and Geman extend this in three ways: (i) general stochastic time changes are mathematically much less constraining than subordinators (they do not require independent, identically distributed increments, which is unrealistic when sampling at irregular dates); (ii) the **cumulative number of trades is a better stochastic clock than volume** for generating virtually perfect normality in returns; and (iii) the clock can be modelled nonparametrically, letting both the time-change and price processes take the form of jump diffusions to accommodate crashes and intense-activity periods.

Our summary: this is a foundational "business-time / transaction-clock" paper and pairs directly with :ref:`The Volume Clock <the-volume-clock>`, which cites it as part of the Mandelbrot-Taylor → Clark → Ané-Geman lineage behind event-based sampling. The mechanism is the one every information-driven-bar user relies on: calendar-time returns are leptokurtic and skewed (fat-tailed), but once you re-index the series by an activity clock that runs fast in busy periods and slow in quiet ones, the conditional return distribution collapses toward Gaussian. Building on Jones, Kaul and Lipson (1994) — who found that it is the *number* of trades, not their size, that generates volatility — and Easley-O'Hara's information-flow work, Ané and Geman argue the trade count is the economic proxy for the information clock. Any arbitrage-free return process can be written as a time-changed Brownian motion (via Monroe's theorem), so identifying the return process reduces to identifying the time change. The honest caveat, documented by a later comment (Murphy and Izzeldin, 2006), is that the procedure recovers the first two moments well but struggles with third and higher moments of the latent information flow, and that returns conditioned on the recentred trade count are not perfectly Gaussian — so treat "virtually perfect normality" as a strong first-order result rather than an exact one.

Used data and code: tick-by-tick transaction data (from Reuters) on two Nasdaq technology stocks, Cisco Systems and Intel; the method builds empirical distributions of trades and returns with classical kernel density estimators, identifies the distribution of the time change through its moments, and reconstructs the return density in transaction time. No public code accompanies this 2000 paper.
...
Per-Level Toxicity Avoidance Learned from a Market Maker's Public Hyperliquid Wallet
------------------------------------------------------------------------------------

Takopi (@OctopusTakopi) reports live-testing a new market-making model on the Binance SOL perpetual, attaching a five-minute screen recording of the run. Two claims make the post worth more than the usual "my bot is printing" genre. First, the author says they did not write a single line of the code — an AI coding agent produced all of it, primed with a large corpus of their own earlier market-making code. Second, and more substantively, the component the author singles out as working "insanely well" is *per-level toxicity avoidance*, and the model behind it was learned from the publicly visible Hyperliquid wallet of Wintermute, one of the largest professional crypto market makers.

**What toxic flow is.** A resting limit order is a free option written to the rest of the market: whoever crosses the spread chooses when to exercise it. Some of that flow is benign — liquidations, hedges, index rebalances, impatient retail — and paying it the spread is profitable because the mid does not systematically move after the fill. The rest is toxic: the taker knows something, or is simply faster, so the mid keeps travelling in the direction of their trade and the maker is marked to a loss within milliseconds. The standard measurement is the *markout*: value the fill against the mid some interval later (in time or in traded volume) and sign it by side. Positive markout means the fill was benign, negative means the maker was adversely selected. Aggregated versions of the same idea are the classical microstructure metrics — Glosten-Milgrom adverse-selection spread components, Kyle's lambda, PIN, and VPIN (see :doc:`Order Flow <./order-flow>` for the VPIN literature). The operationally important fact is that toxicity is not spread evenly across fills: a small minority of fills carries most of the loss, so the binding question for a market maker is not "what is fair value" but "conditional on this quote being hit right now, what is my expected markout" — with widening, skewing, thinning, or pulling as the available responses. The paper *The Market Maker's Dilemma* catalogued in this same file is the canonical demonstration: a naive always-at-the-touch quoter on Binance BTC perps lost nearly 60% in three days, and the repair was precisely a classifier for post-fill reversals.

**What "per-level" adds.** Quoting is not one decision but a decision per price level and per size, and each level has both a different fill probability and a different conditional markout. The touch fills most often and is the most toxic, because it is the first thing a sweep consumes and the only thing a latency-advantaged taker bothers with. Deeper levels fill only when someone is willing to walk the book, which is itself an informative event — sweeps cluster around news and forced flow — but they also come with more spread compensation, so their net markout can be better or worse than the touch depending on the state. The object to be learned is therefore a surface rather than a scalar: expected markout and fill probability as a joint function of distance from mid (in ticks or basis points), queue position, size, book imbalance, the recent signed-trade signature, funding, volatility regime, and cross-venue lead-lag. "Per-level toxicity avoidance" means the quoting policy declines to post at the specific levels whose expected markout is negative in the current state — pulling the touch while leaving depth, or making the ladder deliberately asymmetric — instead of applying one global spread multiplier to everything.

**Why a market maker's public account is the unlock.** Toxicity modelling is fundamentally a labelling problem, and toxicity labels are per-fill, which means they belong to accounts. On a centralised venue you get the anonymised public tape: price, size, timestamp, aggressor side. From that you can build a proxy by pretending you were the passive side of every public trade and computing markouts — this is what most outside-the-firm research does — but you cannot condition on any actual maker's decisions, cannot observe the resting orders that were *not* filled, cannot see inventory or hedges, and cannot distinguish a hypothetical maker from a professional one. Hyperliquid removes that wall because the order book lives on a public chain. Orders, cancels, fills, positions, margin state, and funding payments are all queryable per address through the info API (``l2Book`` snapshots, ``userFills`` / ``userFillsByTime``, ``clearinghouseState``, plus the archived node data), and the wallets of large participants are labelled by explorers such as HypurrScan and Arkham. A labelled market-maker wallet is thus a complete, timestamped, fill-level record of a professional's behaviour on a venue where the matching order book is available for the same interval.

That yields three distinct and complementary training signals. The first is **supervised labels**: for every one of the market maker's fills you know side, size, price, the distance from mid at the moment of execution, and — by joining to the book and trade history — the entire subsequent mid path, so you can construct a genuine (state, level, fill) → markout dataset from a counterparty who really was being adversely selected, rather than from an assumption about who was passive. The second is **revealed policy**: where a top-tier maker chooses to quote, at what depth and size, and above all when it pulls or skews, is the output of a toxicity model that firm has spent years and considerable money building. Fitting a policy to those observed actions is behavioural cloning — a distillation shortcut that lets you inherit the toxicity surface instead of rediscovering it — and unlike fills, quote placements and cancellations are observed continuously, which makes this by far the higher-frequency signal of the two. The third is **absence as a label**: the moments when the maker is missing from a level, or quoting only one side, are exactly the states its own model has flagged as dangerous. Anonymised CEX data contains no equivalent of that observation.

Hyperliquid's transparency also runs the other way, which the post does not mention but which is the natural extension. Because every aggressor is an address with a full history, taker flow can be scored directly: rank wallets by the markout they inflict on whoever was passive, then regress that identity-derived toxicity label onto flow-signature features — trade-size distribution, inter-arrival timing, order type, whether the taker consistently arrives within milliseconds of an oracle or CEX move. The identities themselves do not transfer to another venue, but the features do, which is how an address-labelled dataset becomes a toxicity classifier usable where you are blind.

**The transfer step, and its cracks.** The poster is running on Binance SOL perpetuals, not on Hyperliquid, so the architecture is: train where the labels exist, infer where they do not. That is a legitimate and clever pattern, but it only holds if the learned features are venue-agnostic (book shape, trade signature, cross-venue lead-lag, volatility and funding state) rather than address-based, and several failure modes deserve naming. A cloned policy is optimal for a different game: the maker's Hyperliquid behaviour is tuned to Hyperliquid's fee schedule, tick and lot structure, queue and cancel semantics, latency profile, and — most importantly — Hyperliquid's own flow composition, which is more leverage- and sniper-driven than Binance's. Worse, the visible leg is only one leg: a cross-venue market maker prices Hyperliquid quotes off a consolidated view and hedges elsewhere, so marking its on-chain fills to Hyperliquid's own mid can systematically mismeasure the economics, and imitating one leg of a multi-venue book as though it were a standalone strategy is the classic error in this style of analysis. Attribution is unverifiable too — nothing guarantees a labelled wallet runs a single strategy, and sub-accounts, delta hedges, inventory unwinds, and OTC-related flow all land in the same fill stream. Finally, toxicity is adversarial and non-stationary by construction, since the profitable side of it consists of traders actively hunting makers; a policy fit to last quarter's snipers decays, and because the underlying data is public, any edge extractable this way is being extracted by many people simultaneously.

Treat the evidence accordingly. A five-minute screen recording of a live run establishes that the system exists and was running, not that it is profitable: there are no markout distributions, no fill counts, no per-level realised adverse selection, no inventory or funding costs, and no cost-aware net Sharpe over a sample long enough to contain a regime change. The genuinely transferable idea in the post is not the "agi is here" framing but the data-engineering insight — that an on-chain order book turns a professional market maker's adverse-selection experience into a labelled training set, and that the hard parts of exploiting it are joining fills to book snapshots, aligning clocks across venues, and designing the markout horizon, all of which are exactly the parts an existing personal codebase would already encode when handed to a coding agent.

Posted by Takopi (@OctopusTakopi) in `this discussion <https://x.com/OctopusTakopi/status/2087212750334459907>`__, where the author says the AI-written model's per-level toxicity avoidance — learned from Wintermute's Hyperliquid wallet — "works insanely well".

By Takopi (@OctopusTakopi).

`Read the post <https://x.com/OctopusTakopi/status/2087212750334459907>`__.

Models for the Impact of All Order Book Events
----------------------------------------------

Zoltán Eisler, Jean-Philippe Bouchaud, and Julien Kockelkoren (Capital Fund Management) propose a general framework to describe the impact of different events in the order book, generalising previous work that focused on the impact of market orders alone. Two different modelling routes can be considered, which are equivalent when only market orders are taken into account. One model posits that each event type has a temporary impact (TIM). The "history dependent impact" model (HDIM), on the other hand, assumes that only price-changing events have a direct impact, itself modulated by the past history of all events through an "influence matrix" that measures how much, on average, an event of a given type affects the immediate impact of a price-changing event of the same sign in the future. The authors find in particular that aggressive market orders tend to reduce the impact of further aggressive market orders of the same sign, and increase the impact of aggressive market orders of the opposite sign. They discuss the relative merits of TIM and HDIM, in particular concerning their ability to reproduce the price diffusion pattern, and find that in spite of theoretical inconsistencies TIM appears to fare better than HDIM against empirical data — a paradox ascribed to an uncontrolled approximation used to calibrate HDIMs.

The starting point is the classical propagator model of market-order impact, in which the mid-price is a linear superposition of the decaying impact :math:`G(\ell)` of all past signed trades. Because trade signs are long-range autocorrelated, :math:`G` must decay as a power law for prices to remain diffusive; this decay is really an effective description of the "liquidity refill" by limit orders that trades trigger. The paper shows that a non-fluctuating :math:`G(\ell)` calibrated on the response function accounts for only about two thirds of the long-term price variance on small-tick stocks, so limit orders, cancellations and impact fluctuations must be modelled explicitly. Six event types are defined at the best quotes — market orders, limit orders and cancellations, each subdivided into price-changing (primed) and non-price-changing (zero) variants — and every event is given a sign according to its expected long-term effect on the price. The TIM assigns a bare propagator :math:`G_\pi(\ell)` to each type and is calibrated by inverting the 6 response functions and 36 signed event-event correlation functions. The HDIM instead writes the return at a price-changing event as the sign times a "gap" (half the distance to the second-best quote), and models that gap as a constant plus a linear regression on the past signed order flow via a lag-dependent :math:`6 \times 3` influence matrix :math:`\kappa_{\pi,\pi'}`. The two models coincide only under a restrictive structure of :math:`\kappa`, and the TIM is strictly inconsistent because it assigns non-zero immediate impact to non-price-changing events; the HDIM is the better-founded description, viewing price dynamics as three intertwined processes — instantaneous jumps from events, events inducing further events, and events exerting pressure on the gaps behind the best quotes. The empirical influence matrix reveals that all price-changing events "harden" the book (subsequent same-side gaps shrink) while small non-aggressive market orders surprisingly "soften" it, and this history-dependent asymmetric liquidity is identified as the dominant mechanism that prevents the persistent trends that long-range sign correlations would otherwise produce.

The data set consists of 14 randomly selected liquid NASDAQ stocks over 53 trading days from 3 March to 19 May 2008, with the first 30 and last 40 minutes of each session excluded; the authors note the results were also verified on CME futures, US Treasury bonds and London Stock Exchange stocks. Stocks are split into large-tick names (spread almost always one tick; price-changing events only about 3% of all events) and small-tick names (spreads of several ticks; up to 40% of events change the price). No code is provided and the underlying order book data is proprietary, but the calibration equations are given explicitly, along with the closed-form expressions for the lag-dependent diffusion :math:`D(\ell)` under both models in an appendix, so the method is reproducible on any full-depth order book feed.

This is a theoretical and empirical microstructure paper, not a trading strategy, so there are no return, Sharpe or drawdown figures. The key quantitative results concern how well each model reproduces the observed price diffusion curve :math:`D(\ell)/\ell` on small-tick stocks. The TIM, when calibrated on returns rather than on response functions directly, matches the empirical diffusion surprisingly well at long lags while underestimating it at short lags where high-frequency noise dominates; the constant-gap approximation (no gap dynamics) overestimates volatility, showing that gap adaptation to order flow reduces price volatility. The naively calibrated HDIM overestimates :math:`D(\ell)` by roughly 15% at large lags; multiplying all calibrated :math:`\kappa` kernels by a factor of three greatly improves the fit to both the response functions and the diffusion curve, and a constant high-frequency contribution of about 0.04 ticks squared is needed at small lags. On large-tick stocks the gap fluctuation terms are around two orders of magnitude smaller, so the constant-gap HDIM alone reproduces both the response functions and the diffusion with very good agreement, while the TIM is numerically unstable there because small errors in :math:`G_\pi` are amplified in the variance.

`Read the paper <https://arxiv.org/abs/1107.3364>`__
