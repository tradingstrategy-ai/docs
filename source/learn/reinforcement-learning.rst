...
This paper by Ziyi Wang, Carmine Ventre and Maria Polukarov (King's College London), published at ACM ICAIF 2025 (arXiv:2510.25929), asks whether independently trained market-making bots learn to collude when they share a market. The authors build a hierarchical multi-agent reinforcement learning framework with three functional layers: a top-layer adversary that perturbs volatility and order-arrival intensity to simulate external uncertainty; a mid-layer Agent A, a self-interested market maker trained to maximise inventory-penalised PnL under that adversarial stress; and a bottom layer of competitors — Agent B1 (self-interested, maximises its own PnL), Agent B2 (competitive, trained in a zero-sum setting to minimise its opponent's PnL), and a hybrid Agent B* carrying a learnable modulation parameter omega that interpolates between the two objectives. All agents are trained with PPO via Stable-Baselines3 in a synthetic high-frequency environment with stochastic order flow, probabilistic execution and per-side inventory limits, and critically they observe only their own inventory, cash and the global price — never each other's state or actions.

Beyond aggregate PnL, the paper's main contribution is a suite of interaction-level metrics designed to detect emergent coordination: a herding ratio (synchronous quoting), bid/ask quote distances, inventory divergence, joint drawdown ratio and fill overlap ratio. These are what let the authors distinguish genuine tacit collusion from mere co-existence.

Our summary: the headline result is a negative one, and it is the useful part. Across every pairing tested the herding ratio is exactly 0.000 and quote distances stay wide — there is no evidence of collusion, in sharp contrast to the Calvano et al. (2020) retail-pricing results where Q-learning agents did learn cartel behaviour. The authors attribute this to market structure rather than to agent design: the simulated mid-price follows an exogenous stochastic process, so market makers cannot manipulate price the way retail pricing agents can, and the incentive to collude never materialises. What emerges instead is role differentiation. The zero-sum Agent B2 wins by "paying for priority" — quoting far tighter than its rival (aggressiveness 1.482 vs Agent A's 2.524), sacrificing per-trade margin to capture order flow — and this crowds A down to 16.2% execution share. That aggression tightens the market spread (4.007 vs 4.959 solo), so systemic liquidity improves, but the combined PnL of the pair (342.4) falls well below what A earned alone (506.5): adversarial pressure destroys value even as it improves execution quality. The hybrid B* is the more interesting agent, settling at mean omega ~0.697 (a self-interested lean) and taking 67% market share while leaving its counterpart materially better off than B2 does. For anyone deploying competing RL market makers, the practical read is that reward shaping, not agent capability, determines whether the market ends up efficient-but-fragile or merely crowded.

Two caveats worth weighing. The environment is entirely synthetic with an exogenous price process — the very assumption the authors credit for the absence of collusion — so the no-collusion finding is a property of this market design and does not transfer to venues where market-maker quotes actually move the price. And the reported Sharpe ratios are a per-step PnL mean-over-standard-deviation, not an annualised figure, so they are not comparable to strategy Sharpes quoted elsewhere in this collection.

Data and code: no public code release and no real market data — all results come from the authors' synthetic limit-order-flow simulator (lambda = 400, sigma = 1.1 in the fixed regime; lambda in [300,500] and sigma in [0.2,2.0] under adversarial control). Trained with PPO on Stable-Baselines3.

Key metrics: solo, Agent A earns mean PnL 506.49 (Sharpe 1.749) in a fixed market and 626.42 (Sharpe 1.746) under adversarial control — roughly 24% higher PnL with stable risk-adjusted performance, evidence of genuine generalisation. In head-to-head play, A vs B1 gives 192.70 / 274.56 with market shares 0.373 / 0.627; A vs B2 collapses to 81.48 / 260.95 with shares 0.162 / 0.836 and A's Sharpe dropping to 1.639; B1 vs B2 gives 104.63 / 233.72 with shares 0.243 / 0.756. Against the hybrid, A vs B* is 172.47 / 285.46 (shares 0.329 / 0.671) and B1 vs B* is 215.02 / 218.83 (shares 0.471 / 0.526) — near-parity. Joint drawdown ratio rises from 0.104 (A-B1) to 0.153 (A-B2), and the herding ratio is 0.000 in every heterogeneous pairing, versus 0.467 in the symmetric B1-vs-B1 control.

Added after a search for the two ACM ICAIF 2025 publications claimed by `Stratwise <https://stratwise.ai/#team>`__; those specific papers could not be located in the ICAIF 2025 proceedings, Crossref, OpenAlex, DBLP or arXiv, and this is an unrelated genuine ICAIF 2025 paper on the same topic.

`Read the paper <https://arxiv.org/abs/2510.25929>`__
