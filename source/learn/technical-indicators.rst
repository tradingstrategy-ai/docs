...
**The round trip back to the original.** Two days later the original author reported adopting the whole set of modifications on NQ itself and re-running his own backtest, which closes the loop: the improvements were found on a different instrument in a different codebase and then held on the instrument they came from. The published configuration is now "+100/−100 cross, 9:30–10:00 ET, exit at opposite 61.8 — skip if price is > 2.5 ATR from VWAP", and the reported before/after of adding the VWAP filter to the already-narrowed version is profit factor 2.73 → 3.07, maximum drawdown −$14.0K → −$10.4K, return over drawdown 16.1x → 21.8x, t-statistic 4.93 → 5.18, win rate 64.3% → 65.9%, and trades per year 26 → 24, with total profit on one contract unchanged at +$227K and 2026 year-to-date rising slightly from +$53.6K to +$55.1K. His summary of the filter is that it removed trades that "were basically coin flips" — same money, less risk. Set against the original unfiltered rules, the refined system keeps roughly 87% of the profit on fewer than half the trades (182 since 2019, about 24 a year) with about a third of the drawdown. Asked whether this is simply levered tech beta, he added the long/short breakdown that the earlier posts had not reported: shorts made more than longs, $140K against $87K on one contract, which is difficult to reconcile with a pure long-beta explanation over a period when the index rose throughout. He also restated that per-trade it beats buying every open by roughly 5x and beat all 1,000 of 1,000 random-entry variants in the same window, and said he was re-deploying it in NinjaTrader.
...
**Why it should travel across these particular instruments.** Also our inference rather than a claim in the posts. The agreement between NQ and USTEC is the weakest of the three results as evidence, because they are not independent assets: USTEC is a contract-for-difference wrapper on the same Nasdaq-100 index the NQ future tracks. What that comparison actually tests is implementation and data — a different vendor's bars, a different session definition, a different fee model, two separately written codebases — and it passes, which is worth something, but the underlying flow being traded is identical. The genuinely independent instrument in the exercise is US500, and it is precisely the one that agrees only weakly. That pattern is what the opening-imbalance reading predicts, and it suggests the strategy is not indexed to "US equity futures" generically but to a specific set of conditions that the Nasdaq-100 satisfies unusually well.

Four conditions look necessary for the mechanism, and they rank the instruments in the order observed. First, a discrete cash open that concentrates a night's worth of information into one repricing event — both indices have this, which is why the 9:30 start matters at all. Second, an overnight session that trades continuously but thinly, so that an ATR(14) built from premarket 10-minute bars is materially smaller than regular-hours volatility; this is what lets the ±100 threshold fire early in the session on a move that is large in absolute terms but only three overnight-scaled ATRs. Third, enough underlying single-name and ETF flow that a parent order discovered at the open is worked over hours rather than completed in minutes, which is what supplies the continuation the trade holds for. Fourth, a participant mix that extends the initial move rather than fading it. The Nasdaq-100 scores highest on the last two: it is the most concentrated of the major US indices, its constituents carry the heaviest single-stock options and leveraged-ETF flow — both of which generate mechanical same-direction hedging as the index moves — and it attracts the largest share of momentum-oriented and retail participation. The S&P 500 is broader, deeper, and far more heavily hedged, so opening imbalances are smaller relative to its volatility and are absorbed by cross-sector and index-arbitrage flow more quickly. On this account the 1.14 profit factor on US500 in 2020–23 against USTEC's 1.60 is not a robustness failure to be explained away but the expected ordering: the same effect, weaker where the absorbing liquidity is deeper.

The same reasoning says where it should not be expected to travel, which is the useful part of a mechanism story because it is falsifiable. Continuously traded assets with no opening auction — spot FX, crypto perpetuals — remove the first two conditions outright: there is no premarket-versus-regular-hours volatility ratio for the ATR normaliser to exploit, and no daily moment at which accumulated information is discharged in one move, so the trigger loses the property that makes it fire early on genuine events. Single stocks keep the auction but replace broad institutional rebalancing flow with idiosyncratic news, where the post-open path is dominated by the specific event rather than by an order-working schedule. Smaller or less liquid index products keep the auction but fail the third condition. The concrete prediction is that if the strategy were run across a panel of index futures, performance should sort with the ratio of regular-hours to overnight volatility and with the share of momentum-driven participation — a test neither author has run, and the natural next step for anyone wanting to know whether this is a Nasdaq artefact or a general opening-imbalance effect.

**A third party extends the sample to 2000, and the picture changes.** Cédric Després (@semoi69) re-ran the *original* rules — his trade count of 388 against the author's 392 indicates the unrefined version — and made two changes to how results are presented, both of which matter more than they first appear. The first is a like-for-like sizing comparison over the published 2019–2026 window: one fixed contract against a percentage-of-capital scheme in which 1% of capital is risked per 0.4% of price movement, equivalent to holding a constant notional of about 2.5x capital. Same signals, same 388 trades, same 59.3% win rate, but the derived statistics separate — profit factor 1.85 against 1.71, net P&L +$229,320 against +$175,051, CAGR +18.0% against +15.1%, Sharpe 1.38 against 1.21, Calmar 0.75 against 0.67, at average leverage of 3.1x and 2.5x respectively. The maximum drawdown is the number that reframes the whole exercise: −$30,989, which he quotes as −24.0% of capital, implying an account on the order of $130,000 to carry the position. Stated that way, the strategy is an 18% CAGR at a 24% peak-to-trough loss and a Calmar below 1 — a respectable systematic result, but a different object from the "+$227K on one contract, −$10.4K drawdown, 21.8x return over drawdown" framing, which quietly omits the capital required to run it.

The second change is the more damaging one. He extended the backtest to 2000 and shaded the region the published version never saw, labelling it on the chart as "les 19 années que la stratégie publiée n'a jamais vues" — the 19 years the published strategy never saw. Under percentage sizing the strategy does make money across that span, but the shape is nothing like the post-2019 curve: a strong run from 2000 into a peak around 2008–2009, then roughly seven years of sideways-to-underwater equity through about 2016, spending most of that stretch between −5% and −25% below its prior peak, before resuming its climb into 2019. His own summary is the appropriate one — even with this sizing, you have to be willing to be flat for five or more years and keep taking the trades. He also notes that the two sizings track each other closely from 2019 to 2025 and diverge after, with the fixed contract pulling ahead.

**Why the pre-2019 history looks so different, part one: the sizing hides a price-level trend.** This is our analysis rather than his. Under fixed-contract sizing, dollar P&L is proportional to *point* moves, and a Nasdaq-100 point has been worth $20 throughout while the index itself went from roughly 1,500 in 2003 to roughly 25,000 today. An unchanged percentage edge therefore produces something like fifteen times more dollars at the end of the sample than at the start, purely from the level of the index. That is exactly what his chart shows: the fixed-contract equity line is pinned near zero for the whole 2000–2018 stretch and only takes off after 2019, while the percentage-sized line — which normalises for price level — shows the strategy was demonstrably active and profitable in the early 2000s. The flat blue line is not evidence that the edge was absent before 2019; it is an artefact of quoting a two-decade backtest in undeflated points. The same artefact inflates the recent record, because the steepest part of the fixed-contract curve coincides with the steepest part of the index's rise. This is a distinct problem from the beta question the replication already addressed: the random-entry and buy-every-open controls are also quoted in points, so they correctly establish that the *signal* beats naive entry, but none of them corrects for the fact that the dollar magnitude of the whole exercise scales with the index level. It also explains the divergence after 2025 — a fixed contract at 25,000 index level is simply a larger bet than a fixed contract at 7,000, and the 3.1x versus 2.5x average leverage in the table is that effect measured directly.

**Why the pre-2019 history looks so different, part two: the regime is the edge.** The long flat stretch under percentage sizing runs from roughly 2009 to 2016, and on the opening-imbalance reading proposed above that is precisely where the mechanism should have been weakest. Those were the years of post-crisis quantitative easing: suppressed realised intraday volatility, historically low cross-sectional dispersion, dominant index-arbitrage and passive inflows, and the maturation of execution algorithms whose entire purpose is to absorb and disguise exactly the opening imbalance this strategy is trying to ride. An opening displacement that gets worked off in twenty minutes instead of two hours yields nothing to a system that enters on a 9:30–10:00 cross and holds for continuation. The strong early-2000s performance fits the same story from the other direction — the dot-com unwind supplied enormous, genuinely informational overnight repricings into a market with far less algorithmic absorption. And the post-2019 resurgence coincides with the arrival of the flow that most reliably extends an opening move: 0DTE option hedging, leveraged and inverse ETF rebalancing concentrated in mega-cap technology, and a step-change in retail participation in exactly the names the Nasdaq-100 is built from. Read this way, the 26-year curve is not an indictment of the strategy so much as a confirmation of what it is — a bet on a specific microstructural regime, which paid in 2000–2008, did not pay in 2009–2016, and has paid again since 2019. That is a coherent thing to trade, but it is not a constant, and it comes with the obligation to state which regime you think you are in.

**What this does to the published statistics.** The headline figures from the refined version — profit factor 3.07, return over drawdown 21.8x, t-statistic 5.18 — are all computed inside the favourable regime, on 182 trades from 2019. The t-statistic in particular is doing less work than it appears: it tests whether the mean trade differs from zero within the sample, not whether the sample is representative, and a strategy selected and refined on a regime will produce a strong t-statistic on that regime almost by construction. More pointedly, every one of the improvements — the 10:00 cutoff, the 61.8 exit, the 2.5-ATR VWAP filter — was developed on 2020–2026 data and validated on a train/test split entirely inside that window. The replication's methodology was careful about the split it ran, but a train/test partition of a single regime cannot detect regime-dependence. Cédric Després's contribution is to point out that a genuine 19-year holdout already exists and is sitting unused. Re-running the refined ruleset, in percentage terms, on 2000–2019 is the single highest-value test remaining on this strategy, and until someone does it the honest description of the refinements is that they are unvalidated outside the conditions that produced them. Two limitations of his own test should be noted in fairness: he tested the original rules rather than the refined ones, and sourcing reliable 10-minute Nasdaq futures data with premarket sessions back to 2000 is difficult enough that the pre-2010 portion of any such curve deserves scepticism about the data before scepticism about the strategy.

Caveats the exercise itself surfaces. The sample is small in the way all intraday index systems are — roughly 380 trades over six to seven years — and it spans a period of sustained Nasdaq appreciation, though the beta control partly addresses that. Robustness across instruments is uneven: US500 reaches the same conclusion but only in the second half of the sample, with a 2020–23 profit factor of 1.14 against USTEC's 1.60, which is weak enough to suggest the effect is concentrated in the higher-beta, more retail-driven index. The long/short breakdown was eventually given — shorts $140K against longs $87K — but slippage assumptions beyond "friction included" are never stated, nor is it shown how the results survive the 2020 volatility regime specifically, and the sample thins considerably in the refined version, to 182 trades. And the entire system runs without a working stop, which the MAE study justifies statistically but which remains a live tail-risk decision rather than a solved problem — one whose consequences are easier to see once the drawdown is quoted as −24% of capital rather than as a dollar figure detached from any account size.
