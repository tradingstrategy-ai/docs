...
A plain-English writeup from a retail trader (a former investment professional) covering a specific options breakout strategy, the backtest behind it, the biases that make backtests lie, and how the author uses adversarial AI review to catch them. The strategy is an adaptation of Saty Mahajan's "Bilbo Box": on the hourly chart, wait for the Saty Phase Oscillator to paint grey "compression" candles, draw a box around the first five, and buy a call roughly one strike out of the money with about 28 days to expiry when an hourly candle closes above the box high between 10:00 and 15:00 ET. Three gates must pass — at-least-normal volume for that hour, price above the daily 21 EMA, and a bid-ask spread of 5% of mid or less — and exits are on the stock's price only: a 5-minute close below the box low, a loose trailing stop after a full daily ATR move (giving back up to 75% of open gains), or a 10-trading-day time stop. Every premium-based stop, profit target, and "cut losers on day one" variant tested made results worse, and shorting downside breaks tested at zero to negative.

Across 1,031 trades on 20 megacap names from January 2019 to July 2026, priced against real historical option quotes, the strategy averaged about +11.9% of premium per trade (day-clustered t = 3.56), with a 41% win rate and a median trade of −6.7% — a small number of large winners paying for long strings of small losers. Both halves of the sample were independently profitable (+9.9% in 2019–2022, +13.2% in 2023–2026). Compounding at 4% of account per trade turns $10,000 into roughly $660,000 over 7.5 years with a 43% maximum drawdown; a Monte Carlo resampling of whole trading days shows the median one-year path roughly 1.8x-ing the account and about 1 in 12 paths ending down. The author is candid about the caveats: the exit rule passed a genuine out-of-sample test but the strike and expiry choice was made on the whole record, the ticker list is survivorship-biased, and long-only megacap momentum bleeds in bear markets (2022 was the worst stretch). The strategy is now in a three-month public paper forward test before any real capital is allocated.

The second half is a short primer on what a backtest is — turning a story into a measurable claim with a sample size attached — and the three biases the author sees most: lookahead bias (conditioning on information not available at decision time), survivorship bias (testing on today's winners), and overfitting/data mining (test enough variants and something looks great by chance). The stated goal of a backtest is not to produce a big number but to survive attempts to kill it: year splits, held-out tickers, worst-case fills, cluster-robust statistics, and adversarial code review. On tooling, the author uses Claude Code and Codex for the pipeline work and has each model audit the other's analysis with instructions to hunt for bugs and biases rather than be agreeable, estimating this catches a major flaw at least 90% of the time — including an expiry-anchoring lookahead bug found before the headline number was ever computed.
