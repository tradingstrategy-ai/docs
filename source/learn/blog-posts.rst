...
Rob (@BostonRobTrades) shares his implementation of an Opening Range Breakout strategy for NQ (NASDAQ-100) futures. The strategy trades the break of the first 15-minute range after market open at 9:30 AM, using VWAP as a directional filter: long when price breaks above the range and is above VWAP, short when price breaks below the range and is below VWAP. Entry triggers on a 5-minute candle close outside the range, with exits at end of day or on a 5-minute close back through VWAP or the opposite side of the range. Position sizing is 1–3 contracts, scaled by momentum, range size, and VWAP distance. The author claims a 15-year backtest on this logic.

The ORB concept is one of the oldest intraday strategies in the practitioner literature. Its theoretical basis is sound: the opening period concentrates the highest volume and institutional order flow of the day, and the resulting range reflects where the market aggregates overnight information into a price consensus. Breaking out of that range signals directional conviction. Academic support exists: Zarattini, Barbon, and Aziz (SSRN 4729284, 2024) demonstrate that a 5-minute ORB applied to a filtered portfolio of high-activity U.S. stocks achieved a 2.81 Sharpe ratio and 36% annualized alpha from 2016–2023. Earlier work by the same group on SPY intraday momentum (SSRN 4824172, 2024) showed 19.6% annualized returns and a 1.33 Sharpe from 2007–2024. Timely ORB strategies on five major index futures (DJIA, S&P 500, NASDAQ, HSI, TAIEX) achieved statistically significant 8%+ annual returns from 2003–2013 (IEEE Xplore, 2019).

However, several caveats apply to the strategy as described. First, raw ORB without stock or instrument selection filters has largely been arbitraged away on liquid index futures; the strongest recent academic results come from applying ORB to "Stocks in Play" (stocks with unusual pre-market activity), not broad index futures. Second, the 15-minute opening range is sensitive to regime: on low-volatility days the range is tiny and produces frequent false breakouts, while on high-volatility days the range is wide and the risk-reward deteriorates. Third, using a fixed number of contracts rather than volatility-adjusted sizing means outsized losses on small-range sessions and undersized positions on high-conviction setups; the post mentions scaling by range size and momentum but does not describe a formal risk model. Fourth, the VWAP confirmation filter adds value—it aligns the trade with institutional flow direction—but VWAP in the first 15 minutes is dominated by the opening print and is less informative than later-session VWAP. Fifth, no out-of-sample metrics, transaction cost assumptions, or drawdown figures are provided; a 15-year backtest claim without published equity curves, Sharpe ratios, or maximum drawdowns cannot be independently evaluated.

The broader intraday momentum literature provides context: Gao, Han, Li, and Zhou (Journal of Financial Economics, 2018) show that first-half-hour returns predict last-half-hour returns, consistent with the ORB thesis. Rosa (Journal of Futures Markets, 2022) finds intraday momentum strategies across 62 futures markets achieve Sharpe ratios of 0.87–1.73 at the asset-class level, with stronger performance during recessions. This suggests the ORB edge is real but regime-dependent and likely smaller on a single liquid instrument (NQ) than across a diversified futures portfolio.

Mentioned by Rob (`@BostonRobTrades <https://x.com/BostonRobTrades>`__) in `this discussion <https://x.com/BostonRobTrades/status/2046770986599461326>`__: "Here is a profitable trading strategy that works. ORB - Opening Range Breakout. I have seen many people teach strategies and concepts about trading the opening range but rarely a 15 year backtest on their logic."

`Read the post <https://x.com/BostonRobTrades/status/2046770986599461326>`__.

Mean Reversion on Renko Charts: Exploiting the 70% Ranging Market
------------------------------------------------------------------

A post by Better System Trader summarizing Stefan Friedrichowski's mean reversion framework built on a simple premise: markets spend roughly 70% of their time ranging, not trending, yet most retail trading education, indicators, and strategies are designed to capture directional moves. The proposed solution is to use percentage-based Renko bricks instead of time-based charts. Renko charts filter out time and show only price movement, making mean-reverting markets visible as a clear alternating pattern — up brick, down brick, up brick, down brick. That alternating sequence is framed as the edge: information that is present in the data but obscured by standard time-based chart formats.

The core claim — that markets range 70% of the time — aligns broadly with practitioner intuition and some empirical work on market regimes. Regime-switching models, beginning with Hamilton (1989, "A New Approach to the Economic Analysis of Nonstationary Time Series and the Business Cycle"), have consistently found that financial markets alternate between high-volatility trending states and low-volatility mean-reverting states, with the latter dominating in duration. Ang and Bekaert (2002) and Guidolin and Timmermann (2007) extended this framework to show that the "calm" regime — characterised by lower volatility and range-bound behavior — persists for longer periods across equities, FX, and fixed income. ADX-based studies in the practitioner literature frequently report that readings below 20 (indicating non-trending conditions) occur on 60–75% of trading days for major indices, depending on the lookback period and instrument — broadly consistent with Friedrichowski's 70% estimate. The mean reversion phenomenon itself is well-documented academically: Poterba and Summers (1988) found negative serial correlation in stock returns at longer horizons, Fama and French (1988) decomposed stock prices into permanent and transitory components showing significant mean-reverting behavior, and Lo and MacKinlay (1988) demonstrated short-horizon predictability through variance ratio tests. In shorter-term systematic trading, Connors and Alvarez demonstrated that RSI(2)-based mean reversion strategies on equities produced high win rates (above 70%) historically, though with significant tail risk during regime shifts.

However, the framework as presented has several important weaknesses that the literature highlights. First, Renko charts create a subtle but dangerous **backtesting bias**: a Renko brick does not close until price has moved a full brick size, meaning the closing price of a brick is known only in hindsight. This temporal distortion makes it extremely difficult to construct a realistic backtest — the "alternating pattern" is only visible after the fact, and the entry/exit signals it generates embed look-ahead information that inflates historical performance. Glasserman and Yu (2005) showed that even small sources of look-ahead bias can produce dramatic overstatement of strategy returns. Second, Renko's removal of time information discards valuable signals: time-of-day effects (Gao et al., 2018), volume patterns, and event-driven dynamics all contribute to identifying when mean reversion is likely versus when a genuine breakout is forming — stripping these out reduces the trader's information set. Third, the "alternating pattern" on Renko charts can be **an artifact of brick size selection**: if the brick size is calibrated close to the typical noise amplitude of an instrument, alternating bricks emerge by construction from random walk dynamics — this is a well-known property of threshold-based discretisation of any noisy signal, not evidence of a tradeable mean reversion edge. Anderson (2003) and related work on discretised random walks show that alternation frequencies depend primarily on the ratio of brick size to local volatility, not on any structural market property. Fourth, mean reversion strategies carry **severe tail risk**: they profit from small, frequent reversals but are exposed to catastrophic losses when trends persist — the strategy is effectively short gamma, and the 2008 financial crisis, the 2020 COVID crash, and the 2022 crypto collapse all demonstrated how quickly mean reversion edges can be overwhelmed by regime shifts. Bondarenko (2003) and Dunis et al. (2006) both documented that mean reversion strategies exhibit negative skewness and excess kurtosis in their return distributions. Fifth, the 70% ranging estimate, while plausible, is **unfalsifiable without specifying the exact definition** of "ranging" versus "trending," the timeframe, and the measurement methodology — different ADX thresholds, lookback periods, and instruments yield estimates ranging from 50% to 85%, making the claim more of a heuristic than a testable hypothesis.

Mentioned by `Better System Trader <https://x.com/bettersystrader>`__ in `this discussion <https://x.com/bettersystrader/status/2047331472449720432>`__: "70% of the time, markets aren't trending. They're ranging. Most trading strategies are built for the other 30%."

`Read the post <https://x.com/bettersystrader/status/2047331472449720432>`__.

Deleting Filters to Improve Win Rate: A Forex Structural Breakout Architecture
--------------------------------------------------------------------------------

A LinkedIn post by Iqbal Zainal describing how removing almost all filters from his forex algorithmic trading system — session filters, time filters, and weekend filters — caused the win rate to jump to 75% across 881 live trades over 90 days. The remaining signal logic is a four-component structural breakout system: (1) an H4 trend filter using Directional Indicator crossover on closed bars to prevent repaint bias, (2) a live price breakout above the most recent H4 swing high, (3) an entry distance filter capping the distance from breakout level to 30 pips, and (4) H1 RSI > 40 as momentum confirmation at the moment of breakout. Risk management uses a dynamic stop-loss of max(H1_ATR * 1.5, 25 pips) with a trailing stop that activates at +15 pips and locks at +5 pips.

The reported 90-day metrics are: 881 trades, 75.0% win rate, profit factor 1.50, and maximum drawdown of 3.2%. The author frames the lesson as "simplicity survives; over-optimisation dies in production" — arguing that protective filters were actually skipping profitable trades rather than protecting capital.

The thesis that fewer filters improve live performance has some support in the systematic trading literature. The concept aligns with the bias-variance trade-off well-documented in machine learning and quantitative finance: complex models with many parameters fit training data well but generalise poorly to unseen data. López de Prado (2018, "Advances in Financial Machine Learning") devotes extensive discussion to how additional strategy rules often amount to overfitting, and Robert Carver's "Systematic Trading" (2015) explicitly recommends minimising the number of trading rules, arguing that each additional filter increases the risk of data-mining bias without proportional improvement in out-of-sample performance. Bailey et al. (2014, "Pseudo-Mathematics and Financial Charlatanism: The Effects of Backtest Overfitting on Out-of-Sample Performance") demonstrate that the probability of finding a spurious strategy increases exponentially with the number of parameters tested.

However, several aspects of the claims warrant critical scrutiny. First, **881 trades over 90 days is a limited sample for drawing firm conclusions**. Harvey, Liu, and Zhu (2016, "...and the Cross-Section of Expected Returns") argue that t-statistics should exceed 3.0 (not the traditional 1.96) to account for multiple testing, and White's (2000) Reality Check and Romano and Wolf's (2005) stepwise multiple testing frameworks show that strategy performance must be evaluated against a null of random selection. A 75% win rate with a 1.50 profit factor is not exceptional when considered against the full distribution of possible 90-day outcomes — a simple Monte Carlo simulation would reveal the probability of observing such metrics by chance for any breakout strategy with a moderate edge. Second, **the profit factor of 1.50 with a 75% win rate implies that the average losing trade is roughly 2x the average winner** — this is characteristic of breakout strategies with tight trailing stops that lock in small gains but occasionally get stopped for full ATR-based losses. The asymmetry means the strategy is vulnerable to any regime shift that increases the frequency of false breakouts, even temporarily. Third, **the claim that removing filters improved performance is confounded by the simultaneous bug fix**: the author notes that an absolute value bug in the entry distance filter was causing 60% of prior losses. Attributing the improvement to "simplicity" when a material coding error was simultaneously corrected conflates two distinct effects. Fourth, **90-day drawdown of 3.2% cannot be extrapolated to long-term risk** — Magdon-Ismail et al. (2004) show that maximum drawdown grows as O(sqrt(T)) for diffusion processes, so a 3.2% drawdown over 90 days is consistent with substantially larger drawdowns over a full year or multi-year period. Fifth, the strategy lacks any **regime filter or volatility regime awareness** — forex breakout strategies are well-documented to suffer during low-volatility range-bound environments (Menkhoff et al., 2012, "Currency Momentum Strategies"), and the removal of session/time filters eliminates exactly the kind of structural information that might protect against adverse regimes such as illiquid Asian session breakouts in European pairs.

Mentioned by Iqbal Zainal (`@yllvar <https://www.linkedin.com/in/yllvar/>`__) in `this discussion <https://www.linkedin.com/posts/yllvar_quanttrading-algorithmictrading-python-share-7453244613466705920-SHPe/>`__: "I just deleted almost every filter from my algorithmic trading architecture. The win rate jumped to 75.0%. Simplicity survives. Over-optimization dies in production."

`Read the post <https://www.linkedin.com/posts/yllvar_quanttrading-algorithmictrading-python-share-7453244613466705920-SHPe/>`__.

Regime Detection with Hidden Markov Models for Adaptive Strategy Allocation
----------------------------------------------------------------------------

A LinkedIn post arguing that most strategy failures are not caused by bad signal logic but by regime shifts — structural changes in market dynamics that render historical relationships obsolete. The proposed solution combines unsupervised learning (clustering on returns or volatility features) with Hidden Markov Models (HMMs) that estimate the probability of being in each market regime at each point in time. Rather than producing a binary regime label, the system outputs a probability distribution over possible states, and position sizing becomes a continuous function of those probabilities: reduce leverage when the high-volatility regime probability exceeds a threshold, and shift from mean-reversion to momentum when trending-regime probability rises.

The core thesis — that regime-aware allocation improves long-term survival — is well-supported in the literature. Hamilton (1989, "A New Approach to the Economic Analysis of Nonstationary Time Series and the Business Cycle") established the foundational Markov-switching framework for economic time series, and Ang and Bekaert (2002, "International Asset Allocation with Regime Shifts") demonstrated that regime-switching models improve portfolio allocation relative to static benchmarks. Guidolin and Timmermann (2007, "Asset Allocation Under Multivariate Regime Switching") extended this to multi-asset settings, showing that ignoring regime dynamics leads to substantial welfare losses. More recently, Nystrup et al. (2017, "Dynamic Portfolio Optimization Across Hidden Market Regimes") showed that HMM-based regime detection combined with dynamic allocation meaningfully improves risk-adjusted returns compared to buy-and-hold or static-regime approaches.

However, several aspects of the post warrant scrutiny. First, **HMMs require the number of hidden states to be specified a priori**, and model selection (typically via BIC or cross-validation) is non-trivial — results can be highly sensitive to this choice (Celeux and Durand, 2008). In practice, two-state models (bull/bear or high-vol/low-vol) often work better than richer specifications because additional states tend to overfit in-sample noise. Second, **real-time regime detection lags regime transitions substantially**. The post implicitly assumes filtered probabilities (using only past data) are actionable, but Hamilton's filter updates beliefs gradually — by the time the model assigns 70%+ probability to a new regime, the transition may already be well underway. Bulla and Bulla (2006, "Stylized Facts of Financial Time Series and Hidden Semi-Markov Models") showed that Hidden Semi-Markov Models, which explicitly model regime duration, can reduce this lag, but at the cost of additional parameters. Third, **the suggestion to combine unsupervised clustering with HMMs raises practical questions**: if both methods attempt to identify latent structure, their outputs may conflict, and the post does not address how to reconcile disagreements or avoid double-counting the same information. Fourth, the **70% probability threshold for reducing leverage is presented without justification** — optimal threshold selection is itself a regime-dependent, data-driven problem, and a fixed threshold risks being either too early (reducing exposure during transient volatility spikes) or too late (acting only after a crash is underway). Fifth, **transaction costs from regime-switching strategies can erode returns significantly** if regime changes are detected frequently or noisily. Nystrup et al. (2015, "Regime-Based Versus Static Asset Allocation: Letting the Data Speak") found that after accounting for realistic turnover costs, the outperformance of regime-switching strategies shrinks considerably relative to simpler approaches. Sixth, the post conflates **regime detection** (identifying the current state) with **regime prediction** (anticipating transitions), which is a fundamentally harder problem — an HMM's filtered probability tells you where you likely are, not where you are going, and strategies that react to rather than anticipate transitions may capture little of the theoretical benefit.
...
A Japanese-language portfolio construction case study examining how combining three independently sourced, uncorrelated factor strategies transforms risk-adjusted performance far beyond what any strategy achieves alone. The author reviewed 753 investment strategies sourced from academic papers and selected three whose return drivers are structurally independent: a quality-adjusted size effect, a fundamental-weighted index, and a small-cap multi-factor composite (detailed in the three individual reports below). Each strategy was verified on TOPIX500 data using backtests with statistical significance testing.

When combined with equal weighting and monthly rebalancing, the portfolio compresses maximum drawdown from -13.4% (simple average of the three) to -3.2% — a fourfold reduction — while annualized return rises from +8.4% to +11.3%. Normalizing all approaches to a common -10% drawdown budget (DD-10%) makes the compounding effect stark: the simple strategy average delivers +6.9% annually, while the combined portfolio delivers +26.7%. The article then connects shallow drawdown directly to leverage capacity: with a -3.2% observed maximum drawdown, a 3.2x leverage multiple fits within Japan's margin trading ceiling of 3.3x. After borrowing costs (credit interest 3.0% + securities lending fee 1.2% = 4.1% annualized on the borrowed portion), the leveraged portfolio nets +26.7% — approximately 3.9x the risk-adjusted return of the individual strategy average. On ¥5M capital with a 10% drawdown tolerance, this translates to ¥1.335M annual profit versus ¥345K for the unleveraged average. The central insight is that drawdown reduction is not merely defensive — it creates leverage headroom that converts correlation-based diversification into a compounding multiplier on absolute returns.

The core arithmetic — that combining uncorrelated strategies reduces portfolio variance roughly in proportion to the number of independent sources while expected return remains the sum — is a direct application of Markowitz (1952). What the article adds is the leverage conversion step: diversification compresses required margin, and the freed capacity can be redeployed at the same risk budget. This idea underpins risk-parity frameworks (Qian, 2005; Asness, Frazzini, Pedersen, 2012, "Leverage Aversion and Risk Parity") and the Bridgewater All Weather approach, though those diversify across asset classes rather than equity factor strategies. The "DD-10% normalization" metric used here is structurally equivalent to sizing each strategy to the same Calmar-ratio budget, and the conclusion that leveraged-diversified portfolios outperform unleveraged-concentrated ones matches empirical findings from Clarke, de Silva, and Thorley (2016) on minimum-variance equity portfolios.

However, several assumptions in the leverage arithmetic warrant scrutiny. The -3.2% observed maximum drawdown from an eight-year backtest almost certainly understates the true tail risk — Magdon-Ismail et al. (2004) show observed maximum drawdown grows as O(√T), and the Monte Carlo analyses in the three individual reports show 99th-percentile constituent drawdowns reaching -8.7% to -23.2%. More critically, the correlation between the three strategies — the key variable enabling fourfold drawdown compression — is implicitly assumed stable through time, but factor correlations are well-documented to spike during market stress (Ang and Chen, 2002; Longin and Solnik, 2001), precisely when leverage is most dangerous. The article's leverage arithmetic was also calibrated in a near-zero Japanese rate environment; at higher borrowing costs the net +26.7% figure compresses rapidly, and the margin between strategy gross return and borrowing cost is narrow enough that a 100bps rate increase would reduce net annualized return by roughly 2.2 percentage points given the 2.2x borrowed portion.
...
A verification report applying Asness, Frazzini, Israel, Moskowitz & Pedersen's 2018 paper "Size Matters, If You Control Your Junk" to Japanese TOPIX500 data. The paper argues that the small-cap premium — widely believed to have disappeared after Banz's 1981 discovery — was being suppressed by low-quality small-caps dragging down the factor's average return. When stocks are scored on four quality dimensions (profitability, growth, safety, shareholder returns) and the analysis is run on quality-matched samples, the size premium re-emerges clearly and robustly across geographies and time periods.

The Japanese market backtest (TOPIX500, 2017–2025) confirms the effect with a long-short strategy going long quality small-caps and short large-caps: +8.1% annualized return, 0.88 Sharpe ratio, -12.9% maximum drawdown. The t-test p-value of 0.0362 clears the 5% threshold and the effect holds in both bull and bear market sub-periods. The most recent three years (2023–2025) showed muted results (-0.9% annual return), suggesting the factor may be in a temporary drawdown cycle. Monte Carlo analysis puts the 99th-percentile worst-case drawdown at -20.1%. Overall rating: A (75/100).

The quality-adjusted size effect builds on a long lineage. Banz (1981) documented the original premium; Fama and French (1992, 1993) incorporated it into their three-factor model, though subsequent out-of-sample decades saw it weaken or vanish (Schwert, 2003). The Asness et al. (2018) rehabilitation connects directly to AQR's Quality Minus Junk (QMJ) factor (Asness, Frazzini, Pedersen, 2019), which scores stocks on profitability, growth, safety, and payout — the same four-axis quality score used here. Novy-Marx (2013, "The Other Side of Value") had earlier shown that profitability alone recovers much of the value premium, consistent with the interpretation that low-quality small-caps are a contaminating drag rather than a genuine size exposure. In the Japanese market, Kubota and Takehara (1996) documented a size effect in TOPIX data but post-1990 studies found it unstable, making this 2017–2025 verification a meaningful update for practitioners operating in Japan.

Two critical concerns stand out. First, the recent three-year underperformance (-0.9% annual return, 2023–2025) coincides with a structural shift in Japan: TSE governance reform pressure on low-ROE companies, foreign institutional re-engagement with large-cap exporters, and BOJ policy normalization all favoured large-cap stocks in this period — suggesting the weakness may reflect a macro regime rather than factor decay. Second, the backtest spans approximately 107 monthly observations, which Harvey, Liu, and Zhu (2016, "...and the Cross-Section of Expected Returns") argue is marginal: their recommended minimum t-statistic of 3.0 to account for the multiple-testing problem across published factors corresponds to p<0.003, and this strategy's p-value of 0.0362, while clearing the conventional 5% bar, would fail the more stringent standard they advocate.
...
A verification report applying Arnott, Hsu & Moore's 2005 "Fundamental Indexation" paper to Japanese TOPIX500 data. The paper identifies a structural flaw in market-cap weighting: because stock prices contain noise (temporary over- and undervaluation), market-cap indices mechanically overweight overvalued stocks and underweight undervalued ones. The fix is to weight by real economic scale — sales, cash flow, dividends, book value, employee count — which removes the price-noise feedback loop and produces a persistent tilt toward undervalued stocks. In the original US study (1962–2004), the fundamental-weighted portfolio beat the S&P 500 by ~2% annually with similar volatility and smaller drawdowns. This paper launched the "smart beta" category and underpins the RAFI Fundamental Index family.

The Japanese market backtest (TOPIX500, 2017–2025) delivers +10.8% annualized return, 1.03 Sharpe ratio, and -20.1% maximum drawdown. The t-test p-value of 0.0026 confirms very high statistical confidence. The effect was absent in the first half of the sample (2017–2021, p=0.66) but strong in the second half (2021–2025, +2.26% monthly, p=0.0012), and the most recent three years show exceptional results (+18.3% annual return, 2.03 Sharpe, -6.1% max drawdown). The highest raw return of the three strategies, but also the deepest drawdown at -20.1%, limiting its contribution to the combined portfolio's leverage headroom. Overall rating: B (71/100).

Fundamental indexation generated substantial academic debate after publication. Perold (2007, "Fundamentally Flawed Indexing") argued that any strategy systematically overweighting stocks relative to their market-cap share is simply a value tilt in disguise, with no structural premium beyond what value investing already captures — a critique Arnott and Hsu rebutted by demonstrating that fundamental weights add alpha even after controlling for Fama-French value exposure. Chen, Chen, and Bassett (2007) found the premium largely disappears once value and size controls are applied, while Treynor (2005) provided a complementary theoretical argument that any weighting scheme that severs the link between weight and price will outperform cap-weighting when valuations mean-revert to fundamentals. The aidatalab report's finding of no effect in 2017–2021 (p=0.66) followed by strong performance in 2021–2025 (p=0.0012) is consistent with a regime-rotation story rather than a structural indexing premium: the 2021–2023 global rotation from growth to value, amplified in Japan by TSE reform pressure on persistently low-ROE companies, would benefit fundamental-weighted portfolios regardless of the indexing mechanism.

The deepest concern is sample construction. The report tests on TOPIX500 constituents but does not state whether historical constituent membership is applied retroactively or reconstructed point-in-time — applying today's membership to historical data introduces look-ahead bias, since companies that survived into TOPIX500 by 2025 would artificially inflate fundamental weights for winners in the early sample. The report explicitly guards against information lag (earnings are only used after their official release dates), but this addresses data timing, not constituency survivorship. The missing first-half performance may partly reflect this construction issue, since survivorship-adjusted samples tend to show weaker effects in earlier periods. Prospectively, a concern that applies to all fundamental index strategies is capacity: the rebalancing mechanism that generates the value tilt (selling recent outperformers, buying recent underperformers) creates predictable flows that can be front-run as assets under management grow, a dynamic documented by Frazzini, Israel, and Moskowitz (2015, "Trading Costs of Asset Pricing Anomalies").
...
A verification report applying Alquist, Israel & Moskowitz's 2018 "Fact, Fiction, and the Size Effect" to Japanese TOPIX500 data. The paper systematically addresses six historical criticisms of the size effect — data mining, January effect contamination, liquidity costs, post-discovery disappearance, data errors, and risk explanations — and concludes that standalone size is too weak to be useful as an independent factor. However, the paper's key contribution is showing that size acts as a powerful amplification catalyst: small × value, small × quality, and small × momentum portfolios each produce significantly higher returns than their large-cap equivalents running the same factor exposure. Framed as a sister paper to Asness et al. (2018), it reaches the same conclusion from a different angle.

The Japanese market backtest (TOPIX500, 2017–2025) earns the site's top S-rank (91/100): +6.4% annualized return, 1.40 Sharpe ratio, and only -7.2% maximum drawdown. The t-test p-value is <0.0001 and the effect holds in every sub-period tested — both halves of the sample, bull markets, and bear markets — making it the most statistically robust and regime-consistent of the three strategies. The most recent three years show continued improvement (+8.2% annual return, 1.87 Sharpe, -2.1% max drawdown). The shallow drawdown profile produces the highest risk-normalized return of the three strategies at +8.9% annualized under a -10% drawdown constraint, making it the primary driver of the combined portfolio's leverage capacity.

The "size as catalyst" finding has parallels in earlier literature. Israel and Moskowitz (2013, "The Role of Shorting, Firm Size, and Time on Market Anomalies") showed that many anomalies — including momentum, value, and profitability — are systematically stronger on the short side and among small-caps, because larger stocks are more heavily arbitraged by institutional capital. Hou and Moskowitz (2005, "Market Frictions, Price Delay, and the Cross-Section of Expected Returns") documented that slow information incorporation in less-covered small-caps mechanically amplifies any factor signal that depends on price adjusting to fundamentals. The S-rank (91/100) and <0.0001 p-value are exceptional for an eight-year backtest, but Hou, Xue, and Zhang (2020, "Replicating Anomalies") found that approximately 65% of published factor anomalies fail to replicate with consistent methodology and data, with small-cap-concentrated strategies disproportionately represented among non-replicators — often because their in-sample significance reflects data conditions and liquidity assumptions that do not survive out-of-sample.

The strategy's remarkable Sharpe of 1.40 with only -7.2% max drawdown warrants particular scrutiny regarding transaction costs. Long-short factor portfolios in academic settings achieve such metrics partly because they assume execution at closing prices with no market impact — an assumption that degrades substantially in the less-liquid small-cap segment of TOPIX500. Equal-weight monthly rebalancing generates the highest turnover among the three strategies, since small-cap positions drift more within each period, and Novy-Marx and Velikov (2016, "A Taxonomy of Anomalies and Their Trading Costs") estimated that a significant fraction of small-cap factor returns are consumed by realistic transaction costs even before capacity constraints are reached. The aidatalab report does not include explicit transaction cost modelling, so the reported +6.4% annual return and 1.40 Sharpe represent optimistic upper bounds. Positively, the exceptional regime consistency — statistically significant across both halves of the sample and in both bull and bear markets — distinguishes this strategy from the other two and suggests genuinely diversified factor exposure rather than a single-regime artefact. It is also the natural complement to the Asness et al. (2018) quality-size strategy: where that paper establishes that quality filtering recovers the size premium, this paper establishes the mechanism by which that filtering works in multi-factor portfolios.
