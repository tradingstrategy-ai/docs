...
Pedro Barroso, Paulo F. Maio, and Haoxu Wang (SSRN working paper, 2026) revisit the Moreira and Muir (2017) volatility-management result factor by factor. Applying the same inverse-realised-variance scaling to the nine equity factors used in the original study over 1967–2024, they find that only three — momentum, return on equity (ROE), and betting-against-beta — produce robustly significant positive alphas. The evidence for *portfolios* of factors is considerably stronger: the scaled mean-variance-efficient combination of the five Fama and French (2015) factors produces significantly positive alphas even when no leverage is allowed. The authors attribute the gap between the weak single-factor evidence and the strong portfolio evidence to the role played by conditional correlations among the individual factors.

Our summary: this is a targeted correction to the Moreira and Muir entry at the top of this page, and it changes where a practitioner should expect volatility scaling to earn its keep. The original paper's breadth — the overlay helping nearly every standard factor — turns out not to survive a robustness-focused re-examination at the single-factor level, which is consistent with the broader replication literature on volatility management (Cederburg, O'Doherty, Wang and Yan; DeMiguel, Martín-Utrera and Uppal). What survives, and is arguably the more useful finding, is that the overlay works on the *book*, not on its parts. The mechanism the authors identify is that scaling by realised volatility implicitly cuts exposure when factors become more correlated with each other, since correlation spikes and volatility spikes coincide. That reframes volatility targeting as partly a correlation-management tool in disguise, and it matches how multi-factor books actually fail: not because one sleeve gets noisy, but because the sleeves stop diversifying each other at the worst moment. The practical implication is that a volatility overlay should be evaluated at the portfolio level with diversification benefits included, and that a per-sleeve overlay applied to individual factors is the weaker design.

Data and reproduction: the study uses the standard published factor series — the nine Moreira and Muir (2017) equity factors and the Fama and French (2015) five-factor set — over 1967–2024, so the inputs are freely available from the Kenneth French data library and the Hou-Xue-Zhang q-factor data, and the volatility-scaling construction is fully specified in the original literature. No dedicated code release is advertised. This entry is written from the SSRN abstract and the authors' own summary: SSRN's bot protection blocked full-text retrieval for this collection, so the detailed tables, alpha t-statistics and Sharpe ratios are not verified here.

Key metrics: 3 of 9 volatility-managed equity factors show robust significantly positive alphas over 1967–2024 (momentum, ROE, betting-against-beta); the remaining six do not survive the robustness checks. The volatility-scaled mean-variance-efficient combination of the Fama-French five factors delivers significantly positive alphas under a no-leverage constraint. The paper reports alphas from spanning regressions rather than a tradable-strategy backtest, so no annualised return, maximum drawdown or win rate is given, and the specific alpha and Sharpe figures could not be extracted from the inaccessible full text.
...
By Pedro Barroso, Paulo F. Maio, and Haoxu Wang.
