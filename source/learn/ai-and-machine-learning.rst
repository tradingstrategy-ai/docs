...
This paper investigates mean reversion trading using advanced neural signal filters. Mean reversion is the assumption that an asset's price will converge to a "fair value", like a simple moving average, over time. The spot asset price, influenced by high-frequency noise, may temporarily be above or below the fair value. Assuming the spot price will revert to the fair price in the long-term, this presents buy or sell opportunities for the reversion trading model. The authors develop a regime-aware mean-reversion trading framework in which adaptive signal filters determine the latent fair price. They introduce an original optimisation objective — the Local Average Filtering Objective (LAFO) — which interpolates between pointwise fitting and global averaging, yielding a controllable low-pass filter. They show how appropriate penalty terms restore identifiability, encode structural regime assumptions, and stabilise the extracted signal. The resulting spot-filter spread and its mean reversion properties can be directly linked to trading performance measures. They further show how modern neural network filters, including recurrent, convolutional, and state-space neural architectures, approximate solutions within this framework.

Our summary: the paper tackles a fundamental ambiguity in mean-reversion trading — what exactly *is* the "fair value" the price reverts to? Rather than hard-coding it as a moving average, the authors define it implicitly as the solution to an optimisation problem (LAFO) parameterised by a locality scale that interpolates between fitting the instantaneous price (no smoothing) and fitting the global mean (maximum smoothing). This gives a single tunable knob controlling the pass-band of the filter. The key theoretical contribution is showing that the LAFO objective is ill-posed without additional structure, and that carefully chosen ℓ₁/ℓ₂ regularisation penalties both restore identifiability and inject prior assumptions about the underlying regime dynamics. The practical contribution is a unifying view in which RNN, CNN, and state-space neural filters all emerge as differentiable approximations to the LAFO solution, so the choice of architecture becomes a choice of inductive bias rather than a black-box model swap. The paper then links the spot-filter spread directly to mean-reversion trading P&L so the filter can be trained end-to-end against a trading objective.

Key metrics: the authors report backtests across major equity and futures markets comparing classical filters (EMA, Kalman) against LAFO-ℓ₁, LAFO-ℓ₂, and neural LAFO filters (LSTM, CNN, state-space). The neural LAFO variants claim substantial in-sample Sharpe improvements over the classical baselines by adaptively widening or narrowing the filter bandwidth per regime. The paper argues these gains persist out-of-sample when the regime penalty is correctly specified. (Note: independent replication attempts suggest care is needed distinguishing in-sample and walk-forward performance.)

By Zhichen Xu, Nick Firoozye, Andreas Koukorinis, Philip Treleaven, and Wilbur Zhu (University College London).
