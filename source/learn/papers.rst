...
Yanshu Wang and Luna Wang (Peking University) investigate optimal buy-and-hold portfolio allocation using drawdown-constrained optimization across multiple investment horizons of 5, 10, 20, and 30 years. Using historical ETF data from 1996 to 2026, the paper discovers a remarkably consistent "40/40/20" framework — 40% growth equities, 40% gold, and 20% Bitcoin — that maximizes risk-adjusted returns across all tested time horizons. The optimal maximum drawdown constraint converges to approximately -36% to -38% regardless of horizon length, which the authors interpret as a fundamental structural property of asset allocation. SSRN working paper, February 2026, 12 pages.

Our summary: the paper's main contribution is not the specific weights but the framework: drawdown-constrained optimization consistently lands on assets with genuinely different regime characteristics rather than piling into a single low-volatility bucket. The stability of the 40/40/20 split across horizons from 5 to 30 years is the surprising result, because most portfolio optimization outputs are highly sensitive to the chosen evaluation window. However, the backtested Sharpe ratios of 0.75 to 1.20 should be treated with caution. As Ivan Blanco notes in his discussion of this paper, any optimization result prescribing a hard 20% Bitcoin allocation is unlikely to survive realistic transaction costs, rebalancing drag, and the path dependence of crypto drawdowns. The interesting finding is that drawdown constraints select for regime diversity in the portfolio rather than simple volatility minimization, which connects to the broader theme that regime-aware portfolio construction differs meaningfully from variance-aware construction.

Data and code: historical ETF data from 1996 to 2026 covering growth equities, gold, and Bitcoin. No public code repository was identified.

Key metrics: optimized portfolios deliver Sharpe ratios 2-6x higher than conventional benchmarks including the 60/40 portfolio, the All Weather portfolio, and pure equity strategies. Sharpe ratios range between 0.75 and 1.20 across the tested horizons. The optimal maximum drawdown constraint converges to -36% to -38% across all horizon lengths. The optimal allocation is approximately 40% growth equity, 40% gold, and 20% Bitcoin regardless of the investment horizon tested.

Mentioned by Ivan Blanco (`@ivanblancosanchez <https://www.linkedin.com/in/ivanblancosanchez/>`__), Associate Professor of Finance at CUNEF and Systematic Investing at Noax Capital, in `this discussion <https://www.linkedin.com/posts/ivanblancosanchez_new-research-ideas-a-drawdown-aware-allocation-share-7451962707194535936-5mQl>`__: "Worth reading not for the allocation, but for the framework." Blanco adds: "I'm skeptical of any optimization result that prescribes a hard 20% BTC allocation; those SRs in backtests are hardly sustainable once you account for realistic transaction costs, rebalancing drag, and the path dependence of crypto drawdowns. The interesting part isn't the weights. It's that drawdown-constrained optimization keeps landing on assets with genuinely different regimes, rather than piling into a single 'low-vol' bucket."
...
TradeFM is a 524M-parameter generative Transformer that learns unified trade-flow dynamics directly from billions of trade events across more than 9,000 US equities. The model introduces scale-invariant features and a universal tokenization scheme that map the heterogeneous, multi-modal event stream of order flow into a unified discrete sequence, eliminating asset-specific calibration. Integrated with a deterministic market simulator, TradeFM-generated rollouts reproduce key stylized facts of financial returns including heavy tails, volatility clustering, and absence of return autocorrelation. The model generalizes zero-shot to geographically out-of-distribution APAC markets (Japan and China) with moderate perplexity degradation, suggesting that scale-invariant trade representations capture transferable structure in market microstructure.

Our summary: the paper's core contribution is demonstrating that the foundation model paradigm — pre-train a large model on massive heterogeneous data, then transfer — works for market microstructure. Rather than calibrating separate models per asset (as traditional Hawkes process or agent-based approaches require), TradeFM learns a single model across all liquidity regimes and asset types. The scale-invariant feature design is the key enabler: by normalising prices and volumes relative to local market conditions, the model sees structurally similar patterns regardless of whether it is processing AAPL or a microcap. The closed-loop simulator design is also notable — generated trades feed back into a deterministic order book, so the model must produce sequences that remain self-consistent over rollout horizons. Practical applications include synthetic data generation for illiquid assets, stress testing under counterfactual scenarios, and training reinforcement-learning-based trading agents via interaction with the simulator.

Data and code: the model is pre-trained on a proprietary dataset of billions of tick-level US equities transactions spanning 368 trading days from February 2024 to September 2025, totalling over 19 billion tokens across 1.9 million date-asset pairs. APAC data (Japan and China, January 2025) is used for out-of-distribution evaluation. No public code repository or data was identified; the training data is proprietary.

Key metrics: TradeFM achieves 2-3x lower distributional error than Compound Hawkes baselines across all return intervals. At a 10-event return interval, Kolmogorov-Smirnov statistic is 0.013 for TradeFM versus 0.039 for Hawkes and 0.376 for Zero-Intelligence; Wasserstein distance is 0.0001 versus 0.0004 and 0.0006 respectively. At 120-event intervals, KS is 0.043 versus 0.098 and 0.429; Wasserstein is 0.0007 versus 0.0043 and 0.0043. The model reproduces all tested stylized facts — heavy-tailed return distributions, volatility clustering, and absence of return autocorrelation — across high, medium, and low liquidity tiers. Zero-shot generalization to APAC markets shows moderate perplexity degradation while maintaining stylized fact reproduction.
