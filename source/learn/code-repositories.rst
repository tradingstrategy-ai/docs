...
The Bayesian Grossman-Zhou Rule: Drawdown-Constrained Kelly Betting
-------------------------------------------------------------------

Research code accompanying Sergei Sukhov's working paper *The Bayesian Grossman-Zhou Rule: Drawdown-Constrained Kelly Betting with Parameter Uncertainty* (Market Microstructure Research Lab, June 2026; SSRN 6942459). The problem it tackles: the Kelly criterion prescribes leverage as if the drift μ were known, but in practice μ must be estimated from limited data while trading mandates impose hard drawdown barriers (typically 15–20% from peak), and these two frictions interact — overestimating μ causes overleverage exactly when the barrier is most binding. The paper derives a joint Hamilton-Jacobi-Bellman solution in the state space of log-distance to the drawdown barrier and controlled effective sample size (from a Kalman-Bucy filter), yielding a tractable "Bayes GZ" rule that scales a Bayesian-shrinkage Kelly fraction by the cushion ratio d/b, nests the classical Grossman-Zhou linear cushion rule as sample size goes to infinity, and cuts leverage toward zero as either the barrier is approached or data becomes scarce.

Our summary: a compact, reproducible simulation study rather than a trading library — its value is letting you re-run the paper's central claim that accounting for parameter uncertainty beats plug-in Kelly under drawdown constraints. The Monte Carlo results (50,000 GBM paths) show Bayes GZ's edge grows as data shrinks: roughly neutral with 5 years of estimation data but a +11.2 percentage-point survival improvement and much better penalised utility with only 1 year of noisy data. The README is refreshingly honest about limits, including the stress tests where Bayes GZ *loses* — under Merton jump diffusion and GARCH volatility clustering the conservative leverage reduces growth without improving survival — and a candid revision history that documents retracting an incorrect v1.0 analytic formula for the discounted (r > 0) case, which is proven non-separable. Caveats: it is a single-author, early-stage academic repo (4 stars, created and last pushed June 2026), the empirics are entirely simulated (no real market data), and the code targets figure/table reproduction, not production deployment.

What it means in practice: the whole rule is one position-sizing formula, ``leverage = (Bayesian Kelly fraction) × (distance to your drawdown limit) / (barrier width)``. Two ideas do the work. First, instead of plugging your raw historical estimate of expected return straight into Kelly, you *shrink* it toward a conservative prior, and the less data you have the harder you shrink — so a strategy backtested on one noisy year is sized far smaller than one with a decade of history, which directly guards against the classic failure of Kelly (a lucky sample makes μ look big, you overbet, and you blow up). Second, leverage is tied to how close you currently are to a hard drawdown floor: full size when you are near your equity peak, automatically scaled toward zero as you approach the limit, so you de-risk into losses by construction rather than by discretion. The "double prudence" result just says these two effects reinforce — you bet biggest only when you both have good data *and* a comfortable cushion.

Where you can use it: any strategy where you choose a *size or leverage* on a positive-expectancy edge under a mandated drawdown limit — systematic futures/FX trend and carry books, a crypto perp strategy sized against a max-drawdown mandate, a vol-selling or directional book run for an allocator who will pull capital at a fixed loss, or portfolio-level leverage targeting for a fund with a hard risk budget. It is most useful precisely when your edge is estimated from limited or noisy history (new strategy, new regime, short live track record), because that is where naive Kelly overbets the most and the Bayesian shrinkage adds the most protection. How you would apply it: estimate μ̂ and σ from your data, set the prior (``mu_0``, ``n_0``) and your drawdown threshold δ, compute the current cushion d from your live distance-to-peak, and let the formula output a leverage multiplier that you refresh as equity and the sample size evolve — effectively a fractional-Kelly sizer whose "fraction" adapts to both confidence and remaining risk budget. Two important boundaries from the paper itself: it assumes roughly diffusive (GBM-like) returns, so it *underperforms* plain plug-in Kelly when returns jump or show strong volatility clustering (use a jump/vol-aware extension there), and the clean closed form holds for a zero risk-free rate — the discounted case is only approximate.

Practical details: primary language Python; install with ``pip install -r requirements.txt`` and drive everything through a ``Makefile`` (``make fast`` for a ~10s N=5,000 check, ``make simulate`` for the full ~3-minute N=50,000 run, ``make stress`` for the five return-environment stress tests, ``make figures`` to regenerate Figures 1–7). Code lives under ``code/`` (``simulation.py`` vectorised engine with five strategies, ``stress_tests.py``, ``generate_figures.py``, ``fig7_stress_tests.py``); the README includes copy-paste snippets for computing Bayes GZ leverage and running the simulation directly. Simulation outputs in ``data/`` are generated rather than tracked, and precomputed figures are committed as PDFs. MIT licensed, not a fork, not archived.

`View repository <https://github.com/sergeisukhovmkt/The-Bayesian-Grossman-Zhou-Rule-Drawdown-Constrained-Kelly-Betting>`__

.. _hyper-trees-repository:

Hyper-Trees: GBDTs as Hyper-Models for Classical Forecasting Models
-------------------------------------------------------------------

Official reference implementation of :ref:`Forecasting with Hyper-Trees <forecasting-with-hyper-trees>` (März & Rasul, arXiv:2405.07836) by Alexander März (StatMixedML). Instead of using gradient boosted trees to forecast a time series directly, the library uses a LightGBM-based GBDT to learn the parameters of a classical time series model as functions of features — the classical model then produces the forecast through its own recursion. Available models cover ``Hyper-Tree-AR``/``Hyper-TreeNet-AR``, ``Hyper-Tree-ARMA``/``Hyper-TreeNet-ARMA`` (fitted recursion-free via the two-stage Hannan-Rissanen procedure), ``Hyper-Tree-ETS``, ``Hyper-Tree-STL`` (decomposition), ``Hyper-Tree-VAR``/``Hyper-TreeNet-VAR`` for small aligned panels with cross-series lead/lag dependence, and ``Hyper-Tree-TSB`` for intermittent demand — most usable both as global models trained across many series and as local per-series models.

Our summary: this is the practical entry point to the Hyper-Trees idea, and the API is deliberately low-friction — data needs only ``series_id``, ``date`` and ``value`` columns, any other columns are automatically treated as features, and autoregressive lags are added for you. The properties that make the approach attractive for forecasting work (extrapolation beyond the training range, cross-series learning with per-series parameters, time-varying coefficients readable as classical model parameters) carry over directly, and v0.2.0 (July 2026) added conformal forecast intervals via a ``ForecastIntervals`` helper plus the VAR, ARMA and TSB model families. The ``Hyper-TreeNet`` variants combine tree embeddings with a shallow neural network so runtime stays flat as the target model's parameter count grows — the recommended route for VAR. Caveats: the project is young (36 stars, v0.2.0), full distributional forecasting is still on the roadmap (point forecasts plus conformal intervals today), and the licence is Apache 2.0 *with Commons Clause* — free for research and internal use, but restricting commercial resale of the software itself.

Practical details: Python (LightGBM + PyTorch; GPU recommended but not required for the TreeNet variants), installed with ``pip install hypertrees-forecasting``. The README's quick-start trains a ``HyperTreeAR`` on the AirPassengers dataset in ~15 lines including conformal interval calibration, and the ``examples/`` directory ships notebooks covering hyper-parameter optimisation and the other model families. CI with unit tests and code coverage, released on PyPI (v0.1.0 June 2026, v0.2.0 July 2026), actively maintained, not a fork, not archived.
...
skaters — Online Distributional Time-Series Forecasting by Conjugation
-----------------------------------------------------------------------

Peter Cotton's ``skaters`` is a zero-dependency Python library for online, univariate, *distributional* forecasting of economic time series. Its organising idea is that every model is a chain of bijective transforms with a distributional leaf at the bottom: ``ema``, ``difference``, ``drift``, ``holt_linear``, ``ar``, ``fractional_difference``, ``standardize``, ``garch``, ``seasonal_difference``, ``theta``, ``yeo_johnson`` and ``ou_transform`` each strip one kind of structure out of the series, the leaf fits a Gaussian (or a scale-mixture, for heavy tails) to what is left, and the predictive distribution is pushed back out through the exact inverse chain. Composition is by conjugation, ``f_conj(y) = T⁻¹(f(T(y)))``, so the whole predictive is mapped back rather than a point forecast being back-transformed with the usual bias. Every node returns a ``list[Dist]`` — a weighted Gaussian mixture per horizon — so there is no separate "point forecast" and "uncertainty" API; the mean, the quantiles, the CDF and the log-density are all reads off one object. The package exposes exactly one named forecaster, ``laplace``, a likelihood-weighted Bayesian ensemble over a large candidate population, plus ``dantzig``, a beam search that grows the population online.

Our summary: the interesting claim here is empirical, and it is stated in a way you can check. On thousands of non-price FRED change-series, ``laplace`` wins the per-series held-out log-likelihood race against eleven of twelve baselines — AutoARIMA, AutoETS, R's auto.arima/thetaf/ADAM/nnetar, conformal methods, and zero-shot foundation models — typically on 82–98% of series, and it does so at the fast end of the accuracy/speed frontier. The authors are unusually explicit about where it does *not* win: GARCH-t is a coin flip overall and beats it decisively on price and return series, and the README says plainly that this is not the tool for returns; a period-informed CSP edges it on near-deterministic repeating waveforms. That candour, plus the choice to rank by held-out likelihood rather than a metric the method was tuned for, is what makes the benchmarks worth reading. Two design decisions are worth stealing independently of the package: the "lattice projection" that places near-Dirac atoms on values a series revisits (free on continuous data, a large win on policy rates and posted prices), and the terminal-leaf ensemble, which combines candidates for the mean but lets a single scale-mixture leaf shape the residual — because Bayesian model averaging preserves mean and variance while washing kurtosis out, so simply adding heavy-tailed candidates to a pool does not help. Every predictive also carries a calibration state: the PIT and z-score of each arriving point under the forecast previously issued for it, which is the null model that the companion anomaly-detection work builds on, and the same z stream the project's :ref:`sandwich pattern <the-sandwich-laplace-coordinates>` feeds to third-party models.

Practical details: pure Python 3, ``pip install skaters``, with genuinely zero dependencies (only ``math.erf`` and ``math.exp``), which makes it Pyodide-compatible and runnable in the browser. Parity-locked ports exist in JavaScript (bundled, verified against the Python reference to 1e-6 over ~100,000 values by a parity suite run in CI), R (`skaters-r <https://github.com/microprediction/skaters-r>`__, via r-universe), Julia (`Skaters.jl <https://github.com/microprediction/Skaters.jl>`__), and a Rust core with Python bindings. Reproducible benchmark harnesses live under ``benchmarks/`` beside the results they produced, and a pipeline spec system serialises any composition to JSON and rebuilds it. MIT licensed, not a fork, actively developed (created March 2026, commits within the last day at the time of writing; ~64 stars and ~40 open issues, so expect a moving API). Documentation, methodology guide and interactive demos are at `skaters.microprediction.org <https://skaters.microprediction.org/>`__. Three papers document the library: the methods paper :ref:`Transforms All the Way Down <transforms-all-the-way-down>`, the short software paper :ref:`skaters: Online Distributional Time-Series Forecasting by Conjugation <skaters-joss-paper>`, and the companion empirical study :ref:`An Empirical Study of the Conformal Information Gap <conformal-information-gap>`, which uses the library as its measurement instrument.

`View repository <https://github.com/microprediction/skaters>`__
