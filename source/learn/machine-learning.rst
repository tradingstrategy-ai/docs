...
The authors' central contribution is a unified evaluation framework that judges forecasts not only on statistical accuracy (RMSE, MAE, directional accuracy) but also on economic relevance, via a quantitative bond-trading strategy scored with the Information Ratio, the Omega ratio, and Maximum Drawdown. Their finding is that neural networks consistently outperform classical models on both fronts: traditional models often produce reasonable RMSE/MAE yet generate forecasts that resemble a lagged shift of the current curve, failing to give actionable trading signals, whereas NNs capture nonlinear dynamics and regime shifts (for example, correctly anticipating the sustained 2022 rate increases). The optimal U.S. model is a direct-forecasting NN that uses DNS factors for dimensionality reduction and an autoencoder to extract macroeconomic features, while the best European model is a factor-based NN on PCA-derived rate factors with no macroeconomic inputs — suggesting macro information adds more value in the more volatile U.S. market than in the smoother European one.

The study is reproducible from public data: U.S. rates from the Federal Reserve's FRED database and euro-area rates from the ECB, supplemented by LSEG Datastream (the latter used under licence). A methodological contribution is the reconstruction of a pre-2004 European zero-rate history by splicing German government-bond data onto the ECB AAA curve, achieving an in-sample fit of R-squared above 0.99 across all maturities. No public code repository is referenced.

Key metrics: all six neural-network models selected for in-depth analysis beat the five-year constant-duration benchmark over the evaluation period and achieved a positive Information Ratio, while every classical specification was filtered out for inferior directional accuracy or investment performance despite acceptable error metrics. Forecast errors were consistently lower in Europe (test-set zero-rate-change volatility of 0.10 versus 0.12 in the U.S.), where the strategy hit its duration bounds less often, whereas U.S. models gained the most from autoencoder-compressed macroeconomic features.
...
Yihao Ang, Yifan Bao, Qiang Huang, Qiang Wang, Xinyu Xi, Shuyu Lu, Anthony K. H. Tung and Zhiyong Huang (National University of Singapore, with Qiang Huang also at Harbin Institute of Technology, Shenzhen) present CMS-VAE at ACM ICAIF 2025. The problem they attack is synthetic market data generation for crypto: these markets show extreme volatility, non-stationarity and complex inter-asset dependencies, and the authors argue that existing time-series generation models — GANs, plain VAEs and diffusion approaches — fail on three counts, missing crypto-specific dynamics, generalising poorly, and producing data that is statistically plausible but misaligned with what traders actually need from it.

CMS-VAE makes two changes to the standard VAE recipe. The encoder/decoder use a dilated CNN architecture, chosen to capture long-range temporal dependencies and cross-asset correlations simultaneously. More interestingly, training adds an Ensemble Financial Performance Loss (EFPL), which supervises the generator against an ensemble of trading strategies rather than against distributional distance alone — the synthetic series is pushed to be strategy-consistent and risk-aligned, not merely to look like real data. The authors evaluate along three axes: generative fidelity, downstream predictive modelling, and statistical arbitrage performance.

Our summary: the notable idea here is EFPL, and it is worth separating from the architecture. Most synthetic-market-data work optimises a fidelity metric — discriminative score, marginal distributions, autocorrelation structure — and then hopes the result is useful for backtesting. That is the wrong objective if the purpose is strategy development: a generator can match unconditional statistics well while destroying exactly the structure a strategy trades on. Supervising directly against an ensemble of strategies inverts this, making "would a strategy behave on this synthetic data as it does on real data" part of the training signal. For anyone using synthetic data to stress-test or augment a crypto backtest, that framing is the transferable lesson regardless of whether you adopt this specific architecture. The ensemble matters too: supervising against a single strategy would simply overfit the generator to that strategy's signal.

Caveat on this entry: the paper is closed-access in the ACM Digital Library, which blocks automated retrieval, and there is no arXiv or open-access copy — the OpenReview record requires interactive challenge verification. The summary above is therefore built from the authors' abstract and the verified publication record, not a full read of the paper. The mechanism description is what the authors claim; the internal details of EFPL, the baseline set, the data window and the arbitrage backtest protocol have not been independently checked. Treat the metrics below as reported rather than assessed.

Data and code: not verified. No open-access PDF, no public code repository located, and the datasets and evaluation windows are not stated in the abstract. Retrieval was attempted via ACM DL, OpenReview, arXiv, Semantic Scholar and Unpaywall.

Key metrics: the authors report that CMS-VAE consistently outperforms state-of-the-art baselines across generative fidelity, predictive modelling and statistical arbitrage, achieving up to 96.8% lower prediction errors and a 1.4x improvement in Sharpe ratio. Note "up to" — these are best-case figures over the evaluation suite, not averages. Supporting context for the group's standing on this problem: the same lead authors won first place in the ACM ICAIF 2024 Cryptocurrency Market Risk Assessment Simulation Competition, and the work was also presented at the NeurIPS 2025 Generative AI in Finance workshop.

Added after a search for the two ACM ICAIF 2025 publications claimed by `Stratwise <https://stratwise.ai/#team>`__; those specific papers could not be located in the ICAIF 2025 proceedings, Crossref, OpenAlex, DBLP or arXiv, and this is an unrelated genuine ICAIF 2025 paper on crypto market modelling.

`Read the paper <https://dl.acm.org/doi/10.1145/3768292.3771253>`__
Tabular Deep Learning for Algorithmic Trading: Cross-Regime Bayesian Optimisation for Equity Signal Generation
-----------------------------------------------------------------------------------------------------------------

Josh Le Grice compares logistic regression, XGBoost, a multilayer perceptron, TabNet, and an FT-Transformer for daily cross-sectional equity prediction. The task classifies the next day's return percentile for about 300 large-cap US equities into short (bottom decile), hold (middle 80%), and long (top decile). Model and portfolio hyperparameters are selected through Bayesian optimisation across three expanding-window validation regimes (2022--2024), explicitly rewarding annualised return and Sharpe ratio while penalising drawdown and negative individual-regime results. The strongest configuration is a rank-aggregated Hybrid of XGBoost and TabNet, chosen for complementary signals rather than a claim that TabNet alone beats trees.

Our summary: this is a well-specified cross-sectional, daily market-neutral signal study. Each day the model ranks long and short probabilities separately, buys the top six and shorts the top six, equal-weights positions within each book, and rebalances at the close with 100% gross dollar-neutral exposure. The key contribution is treating regime robustness as a selection constraint rather than choosing models on one validation period. That is a sensible protocol, but it does not isolate the incremental benefit of cross-regime tuning because the study does not run an otherwise identical single-regime-tuned baseline. The paper also acknowledges that its historical end-of-sample S&P 500 membership creates survivorship bias, that 2025 is only one 252-day holdout, and that selecting from roughly 161 configurations without a Deflated Sharpe Ratio makes the reported probabilistic-Sharpe evidence an upper bound.

Data and reproduction: the sample covers 2015--2025 and combines Bloomberg OHLCV, fundamentals, and news sentiment with FRED macroeconomic data and Google Trends. Macro, fundamental, news, and search features are lagged to mitigate look-ahead; 2025 is held out after development on 2015--2024. The backtest applies a fixed 2.2 bp cost per traded leg, but omits stock-specific short borrow fees and availability, which likely overstates short-side returns. Bloomberg's licence limits independent replication, and the paper links no public source-code repository or frozen input data. An interactive application is described, but it is based on the fixed historical dataset rather than a live deployable system.

Key metrics: on the 2025 holdout, the Hybrid reports 51.26% return, Sharpe 2.44, Sortino 5.35, Calmar 6.60, and maximum drawdown -7.76%; the strongest individual model, XGBoost, reports 33.61% return and Sharpe 1.37. Hybrid CAPM alpha is 0.423 annually (p = 0.011), beta is 0.048, and its probabilistic Sharpe ratio against the S&P 500 is 0.960. These point estimates should be read with the paper's stated survivorship, multiple-selection, one-year-holdout, and short-borrow-cost limitations.
