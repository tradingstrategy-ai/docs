...
Data and code: simulation studies plus real-world financial data experiments (the abstract does not enumerate the specific markets). The article is paywalled (Annals of Operations Research, Springer) with no open-access manuscript available, so this entry is based on the abstract rather than the full text. The work is associated with the ``jumpmodels`` Python package maintained by co-author Yizhan Shu.

Key metrics: the abstract reports that the continuous jump model outperforms traditional regime-switching models, with the outperformance pronounced under imbalanced regimes and limited historical data. Specific numerical results (accuracy, Sharpe, or error metrics) are not available without the full text, which is behind a publisher paywall.

Critical view: the conceptual contribution — probabilistic regime membership plus a simplex-vertex mode-loss penalty — is a natural and useful generalization, and the emphasis on imbalanced-regime, small-sample performance addresses a real weakness of discrete HMMs and jump models. However, this summary is necessarily based on the abstract alone; the empirical evidence, choice of benchmarks, and any trading or risk backtest could not be verified because the PDF is paywalled and the paper is too recent to retrieve via open mirrors. The claims should be regarded as promising but unverified pending access to the full text.
