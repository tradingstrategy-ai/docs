...
.. _learning-hmm-penalizing-jumps:

Learning hidden Markov models with persistent states by penalizing jumps
------------------------------------------------------------------------

Peter Nystrup, Erik Lindström, and Henrik Madsen (Lund University and Technical University of Denmark; Expert Systems with Applications 150, 2020, article 113307) introduce the *statistical jump model* — the foundational method underpinning the whole sparse/continuous jump-model literature. When a hidden Markov model is misspecified or misestimated it tends to switch states unrealistically fast; the authors address this by reframing state inference as clustering of temporal features while explicitly penalizing jumps between states. The jump penalty is a hyperparameter that directly controls the transition rate, and the model is fit by a fast coordinate-descent procedure that learns the state sequence and parameters simultaneously rather than by maximizing the likelihood with the EM algorithm.

Our summary: this is the seminal jump-model paper that the later "sparse jump model" (feature selection) and "continuous jump model" (probabilistic states) extend. Its central insight is that penalizing jumps acts as a regularizer that enforces state persistence, which matters enormously in production systems where spurious regime flips create churn. Compared with maximum-likelihood estimation and spectral clustering across an extensive simulation study, the jump estimator converges in far fewer iterations, is much less sensitive to initialization, performs relatively better as the number of states grows, and — because it makes no Gaussian assumption — is robust to heavy-tailed conditional distributions that severely bias the MLE's transition probabilities.

Data and code: simulation study plus an application to daily S&P 500 total-return index data from 2009 to 2018. The benchmark Gaussian HMM is fit with the ``hmmlearn`` Python package. The paper predates and motivates the later ``jumpmodels`` package; no dedicated code release accompanies this article.

Key metrics: in a long–short S&P 500 strategy used to illustrate the value of correctly estimated persistence, the jump estimator achieved a Sharpe ratio of 1.434 versus 0.923 (spectral) and 0.673 (MLE), while cutting turnover to 1.392 versus 11.03 (spectral) and 4.175 (MLE). The resulting break-even transaction cost was 0.0736 for the jump estimator versus 0.0089 (spectral) and 0.0169 (MLE) — roughly an order of magnitude more tolerance to trading costs because the inferred state sequence is far more persistent.

Critical view: the authors are careful to state that the trading application is meant to compare estimation methods, not to prove profitability — the S&P 500 results are in-sample and would require out-of-sample testing to support an alpha claim. The main acknowledged limitation is the forward-looking nature of some temporal features (they are constructed using windows that can peek ahead), and like all regime methods the smoothed state estimates carry detection lag in real time. Treated as a methodological contribution to robust, persistent regime estimation rather than a strategy, the results are convincing and have been widely adopted.

`Read the paper <https://www.sciencedirect.com/science/article/abs/pii/S0957417420301329>`__

.. _feature-selection-in-jump-models:
...
`Read the paper <https://www.sciencedirect.com/science/article/pii/S0957417421009647>`__

.. _what-drives-cryptocurrency-returns-sparse-jm:
...
`Read the paper <https://link.springer.com/article/10.1007/s42521-023-00085-x>`__

.. _continuous-statistical-jump-model:
...
Synthetic generators of daily equity returns let practitioners stress-test, backtest, and design scenarios that a single realised market history cannot supply — but only if the generator reproduces the stylised facts of real returns: heavy tails, negligible linear autocorrelation, and slow decay of the absolute-return autocorrelation. Hidden Markov models with a few Gaussian states were long thought unable to reproduce that slow decay (the Rydén–Teräsvirta–Åsbrink 1998 result), and the standard fix was to abandon them for more complex hidden semi-Markov models. Abdulrahman Alswaidan, Cade Jin and Jeffrey D. Varner (Cornell, 2026) revisit the question with a continuous-emission hidden Markov model (CHMM) whose latent regime chain governs the autocorrelation while per-regime densities govern the marginal, separating the temporal and distributional sides of the original failure. A unified expectation-maximisation framework fits Gaussian, Student-t, Laplace, and generalised-error emissions under shared forward-backward recursions and quantile-based initialisation, and a spectral identity bounds the number of ACF decay modes by the rank of the centred transition matrix.

Our summary: the paper's central claim is that the classic low-state HMM failure is *distributional, not temporal*. A standard closed-form identity writes the absolute-growth-rate ACF as a sum of geometric modes at the non-unit eigenvalues of the transition matrix, bounding the number of decay modes by the matrix rank — but on the data studied that bound was not binding once K ≥ 3 states were used. An effective-rank diagnostic found a single non-unit eigenvalue carrying 93.6% of the lag-1 absolute-ACF at K = 18 on SPY, so what actually closed the fit gap was a heavy-tailed *emission* family, not extra decay modes. The authors select K* = 3 by held-out log-likelihood, BIC, and rolling-origin cross-validation, then show the fitted model supports two things a plain i.i.d. bootstrap cannot: a regime-conditional Value-at-Risk built from the model's one-step-ahead state forecast, and a Student-t copula that couples several single-asset CHMMs into a joint multi-asset generator while leaving each fitted marginal untouched. The honest framing is notable — an i.i.d. bootstrap and a maximum-likelihood hidden semi-Markov model actually beat the CHMM on raw single-window distributional fit (Kolmogorov-Smirnov), and the CHMM's advantage is specifically the regime-conditional risk forecast and multi-asset composition that those baselines cannot provide.

The work is candid about scope limits. A single in-sample fit does not generalise across *regime introductions*: a non-equity stress test on the gold ETF (GLD) collapsed under static fitting, a CRSP cross-decade transfer (a calm 2004–2006 slice sitting outside the 1994–2004 training regimes) fell to a 3–5% pass rate, and the COVID-2020 and 2022 rate-hike walk-forward folds were rejected for every generator in the panel — so periodic refit is recommended, at a cadence judged against the out-of-sample slice rather than the in-sample fit. Only the three *symmetric* Cont stylised facts are targeted; the leverage effect is left to future work because symmetric emissions cannot reproduce a negative return–volatility cross-correlation by construction.

Used data and code: daily SPY from 2014-01-03 to 2024-01-03 (in-sample, 2,516 trading days) with out-of-sample 2024-01-04 to 2026-04-20 (572 days), plus a sector-balanced 30-ticker GICS panel, a CRSP 1994–2004 → 2004–2006 cross-decade transfer, and a six-asset US-equity basket (SPY, NVDA, JNJ, JPM, AAPL, QQQ); prices from Polygon.io and Alpaca/IEX, CRSP under institutional licence (not redistributed). The models and full evaluation pipeline are implemented in Julia and released under an MIT licence: the paper repository (`CHMM-Paper-Repository <https://github.com/varnerlab/CHMM-Paper-Repository>`__) and the core model library (`CHMM-Model-Repository <https://github.com/varnerlab/CHMM-Model-Repository>`__).

Key metrics: this is a generator/risk-model study rather than a trading strategy, so the headline numbers are statistical-fidelity and VaR-coverage results rather than P&L. On SPY (1,000 simulated paths; observed excess kurtosis 7.68 in-sample / 5.29 out-of-sample), the shared-ν Student-t CHMM at K* = 3 gave the cleanest heavy-tail match with no tuning hyperparameter — Kolmogorov-Smirnov pass rate 91.9% IS / 82.1% OoS and simulated kurtosis 4.68 / 4.46 — versus the Gaussian CHMM-N at 91.5% / 78.0% and kurtosis 3.83 / 3.62 (Table 1). An i.i.d. bootstrap (KS 99.8% / 91.8%) and an ML hidden semi-Markov model (98.4% / 91.0%) scored higher on raw single-window KS but cannot serve the downstream risk and multi-asset use cases. For risk, the regime-conditional VaR passed the Christoffersen joint conditional-coverage test at α = 0.05 on the main out-of-sample window (CHMM-N at K* = 3: 35 breaches, a 6.12% breach rate against the 5% nominal, and it also passed the Engle–Manganelli dynamic-quantile test, p = 0.16) and at α = 0.01 (9 breaches, 1.57%); it passed Christoffersen-cc on 19 of 24 rolling walk-forward rows, the five failures concentrated on the COVID and 2022 rate-hike folds, and it beat filtered-bootstrap and CAViaR contenders on the strict α = 0.01 tail. The six-asset Student-t copula (ν* = 6) reproduced off-diagonal correlations with a mean absolute error of 0.027 in-sample and 0.209 out-of-sample, tightening to 0.185 under quarterly refit.
...
This paper by Matthew Wang, Yi-Hong Lin and Ilya Mikhelson (Northwestern University and National Taiwan University; *Journal of Risk and Financial Management* 13(12):311, 2020, doi:10.3390/jrfm13120311) uses a hidden Markov model to classify US market regimes and then rotates between factor investing models depending on which regime is currently detected. The HMM is a three-state Gaussian model fitted with hmmlearn on S&P 500 ETF OHLC data, using just two observation variables: daily return and a 10-day rolling volatility measure. A full covariance matrix is used rather than a diagonal one, so cross-correlation between return and volatility is retained. Three states were chosen because the log-likelihood gain flattens after the two-to-three-state step; the fitted model produces an average regime duration of about 12.5 days.

The three states come out as recognisable market types: regime 1 is a steady bull (42.4% of days, average daily return 0.046, volatility 0.94), regime 0 is a sideways "kangaroo" market (42.4%, return 0.040, volatility 3.47), and regime 2 is a high-volatility bear (13% of days, return -0.066, volatility 13.6). Transition probabilities are strongly persistent on the diagonal (0.90, 0.94, 0.89). Six factor models were built on QuantConnect — Fama-French three-factor, a modified Fama-French, Carhart four-factor, a long-only Value model, an AQR-style quality/value/momentum model, and the S&P 500 ETF as benchmark — backtested January 2007 to September 2017 to establish which model performs best in which regime. Live, the HMM is retrained daily on a 2707-day sliding window, and a regime switch fires only when Kolmogorov-Smirnov-fitted PDFs clear hand-set thresholds (volatility PDF > 0.3 and return PDF > 0.5).

Our summary: the mechanism is sound and the paper is unusually clear about it — this is a good reference implementation for anyone wiring an HMM into a factor-rotation overlay, and the regime statistics table alone is worth the read. The authors are also honest about the framework's ceiling, noting that returns remain "heavily contingent on and limited by the inherent quality of the factor models the HMM is based on" — the HMM diversifies across factor exposures, it does not manufacture alpha. Their own Treynor-Mazuy market-timing coefficients are not statistically significant in any model, including theirs, which they attribute to the portfolios' risk exposures being distinct from plain market risk.

The headline result needs heavy discounting, though. The out-of-sample window is September 2017 to April 2020 — roughly 2.6 years that terminate precisely on the COVID crash, and the authors' own qualitative section concedes the strategy underperforms the Value model during bull runs and earns its advantage in the December 2018 and March 2020 drawdowns by rotating from a twice-leveraged long-only Value model into a market-neutral Fama-French one. So a large share of the reported edge rests on two events, one of them the test period's final month. Compounding this, the regime-to-model mapping was chosen by inspecting which factor model performed best in each regime during the training period, meaning the "out-of-sample" test inherits an in-sample selection step. No transaction costs, slippage or turnover are discussed anywhere in the paper, and the factor models use convenience proxies (price-to-book in place of book-to-market, price-to-earnings as a stand-in for excess market return) because QuantConnect lacked the canonical fields. Read this for the architecture, not for the performance numbers.

Data and code: S&P 500 ETF OHLC data for the HMM; factor models built and backtested on QuantConnect; Fama-French factors (MKT, SMB, HML, MOM) from the Ken French data library and QMJ from Andrea Frazzini's library for the attribution regression. No public code repository and no data availability statement — reimplementation is possible from the described rules but nothing is released. Open access under CC BY.

Key metrics: over the September 2017-April 2020 out-of-sample period the HMM rotation model reports a Sharpe ratio of 2.017, information ratio 1.64, Treynor ratio 0.264, cumulative return 2.4491 and maximum drawdown 0.1283. Against it: the S&P 500 returned -0.0068 with Sharpe -0.174 and a 0.341 maximum drawdown; the best individual factor model (Value) returned 0.5318 with Sharpe 0.463 but a 0.5356 maximum drawdown; Fama-French three-factor returned -0.2115 (Sharpe -1.418) and the AQR model -0.2535 (Sharpe -1.423). A factor-attribution regression against MKT/SMB/HML/MOM/QMJ gives alpha of 0.1723 with t = 3.308, which the authors describe as roughly 2% annual abnormal return. The Treynor-Mazuy gamma of 0.3698 is *not* statistically significant, so the paper's own market-timing evidence is inconclusive.
